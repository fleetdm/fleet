...
for system in $SYSTEMS; do
	...
	GOOS=$goose_value GOARCH=arm64 go build -ldflags="-X github.com/fleetdm/fleet/v4/orbit/pkg/build.Version=42" -o $orbit_target ./orbit/cmd/orbit
	...
	if [[ $system == "macos" && -n "$MACOS_TEST_EXTENSIONS" ]]; then
		for extension in ${MACOS_TEST_EXTENSIONS//,/ }; do
	...
	if [[ $system == "linux" && -n "$LINUX_TEST_EXTENSIONS" ]]; then
		for extension in ${LINUX_TEST_EXTENSIONS//,/ }; do
			extensionName=$(basename $extension)
			extensionName=$(echo "$extensionName" | cut -d'.' -f1)
			./build/fleetctl updates add \
				--path $TUF_PATH \
	...
	if [[ $system == "windows" && -n "$WINDOWS_TEST_EXTENSIONS" ]]; then
		for extension in ${WINDOWS_TEST_EXTENSIONS//,/ }; do
			extensionName=$(basename $extension)
			extensionName=$(echo "$extensionName" | cut -d'.' -f1)
			echo "$FILE" | cut -d'.' -f2
			./build/fleetctl updates add \
