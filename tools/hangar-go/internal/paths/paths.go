...
//
// During the Rust->Go build-alongside phase this is deliberately the
// "-go" variant so an in-development Go build keeps its settings and
// running.json separate from the user's working Rust Hangar — a buggy dev
// build can't corrupt the real app's state. At cutover, flip this to
// "com.fleetdm.fleet-hangar" and existing users' settings carry over
// untouched.
const BundleID = "com.fleetdm.fleet-hangar-go"
