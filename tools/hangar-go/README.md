...
A desktop control panel for [Fleet](https://github.com/fleetdm/fleet) contributors —
the Go/[Wails 3](https://v3alpha.wails.io) port of the original Rust/Tauri app in
[`tools/hangar`](../hangar). Bundles the daily tasks of working on a Fleet clone (checking
out branches, building, running `fleet serve`, tailing logs, managing the dev MySQL, driving
`fleetctl`, applying GitOps repos, spinning up `osquery-perf`) into one app. macOS-first.
...
- **Bundle identifier** is currently `com.fleetdm.fleet-hangar-go` so this in-development
  build keeps its settings/`running.json` separate from the original Rust app while both
  exist. Settings live under `~/Library/Application Support/<id>/` and logs under
  `~/Library/Logs/<id>/`; DB backups live in `<repo>/db-backups/`. At cutover this flips to
  `com.fleetdm.fleet-hangar` so existing users' settings carry over.
