...
| `fleet-hangar/` | Desktop control panel for the Fleet dev environment (Tauri, macOS) — run `fleet serve`, manage branches / DB / fleetctl / GitOps / osquery-perf from one window | `cd tools/fleet-hangar && npm install && npm run tauri dev` - See [fleet-hangar/README.md](fleet-hangar/README.md) |
