...
| `hangar/` | Desktop control panel for the Fleet dev environment (Tauri, macOS) — run `fleet serve`, manage branches / DB / fleetctl / GitOps / osquery-perf from one window | `cd tools/hangar && npm install && npm run tauri dev` - See [hangar/README.md](hangar/README.md) |
