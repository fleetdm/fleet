...
| `MCP_RATE_LIMIT_MODE` | `global` | SSE rate-limit backstop (see [Security model](#security-model)). `global` = one shared bucket (right behind a proxy/WAF, incl. **Render**); `ip` = one bucket per client IP, keyed on the TCP peer (for direct-connect internal networks — never use behind a proxy). |
...
- **Rate limit on SSE (a flood backstop, not the brute-force defense — that's the strong `MCP_AUTH_TOKEN`).** Sheds request floods (including failed-auth requests, rejected before they reach Fleet) and keeps the MCP from amplifying load onto Fleet; returns `429 Too Many Requests` with `Retry-After: 1` on overflow. Two modes via **`MCP_RATE_LIMIT_MODE`**:
  - **`global`** *(default)* — one shared token-bucket (burst 100, 25 req/sec). Right for a small trusted operator group and for **any deployment behind a reverse proxy / WAF** (where per-client throttling belongs at the edge — e.g. Cloudflare; Render's edge gives client IP + coarse DDoS protection but no configurable per-IP limiter).
  - **`ip`** — one bucket per client IP (burst 40, 10 req/sec each), for a **direct-connect internal-network** deployment, so one noisy/compromised client doesn't throttle the rest. It keys on the **TCP peer** (`RemoteAddr`) and **never reads `X-Forwarded-For`** — the TCP source of an established connection can't be spoofed, whereas XFF is client-supplied (this is the Finding C bypass, designed out). Trade-off: **behind a proxy** every request's `RemoteAddr` is the proxy's IP, collapsing all clients into one bucket — this fails *safe* (over-throttles), but use `global` there.
...
- **Read vs write = the token's Fleet role.** The MCP acts with exactly the role of `FLEET_API_KEY` (it has no mode of its own), and Fleet enforces it: an **observer** token is effectively read-only (Fleet `403`s `create_saved_query` / `run_live_query`). Per Fleet's RBAC, `create_saved_query` needs **admin / maintainer / gitops**, and `run_live_query` needs **admin / maintainer / technician / observer_plus**. Recommended: an API-only user with the **lowest role** your tools need (observer for read-only deployments). The MCP doesn't infer or log these rights — it only enforces the API-only requirement above.
...
tools/fleet-mcp/
  ...
  rate_limit.go            # SSE throttle: global (default) or ip mode (MCP_RATE_LIMIT_MODE)
