...
| `FLEET_API_KEY` | *(required)* | Fleet API token — see [Fleet docs](https://fleetdm.com/docs/using-fleet/rest-api#authentication). May alternatively be supplied via `FLEET_API_KEY_FILE`. **Use the least-privileged Fleet role that covers your tools:** an **observer** / observer-plus token is enough for all read-only tools. For deployments that use the two write tools (`create_saved_query` + `run_live_query`), a **maintainer** covers both — **admin is not required**. With an observer token, Fleet itself rejects (403) the mutating calls, so the MCP is effectively read-only. An admin token only widens the blast radius of a leaked credential. |
| `FLEET_API_KEY_FILE` | *(optional)* | Path to a file containing the Fleet API token. Preferred over `FLEET_API_KEY` for production: keeps the Fleet token out of process env (`ps`), shell history, and `claude_desktop_config.json` (readable by your UID, lands in Time Machine backups). When both are set, `_FILE` wins. |
...
This means Claude Desktop will surface a confirmation prompt before either tool fires — required for any production deployment where the operator's Fleet API token can write (maintainer / admin).

## Security model

The MCP holds the operator's `FLEET_API_KEY` and acts with exactly that token's Fleet role — with a write-capable role (maintainer / admin), compromise of the MCP gives an attacker full host inventory access plus arbitrary osquery against every enrolled device. Defenses:
...
- **Read vs write = the token's Fleet role.** The MCP acts with exactly the role of `FLEET_API_KEY` (it has no mode of its own), and Fleet enforces it: an **observer** token is effectively read-only (Fleet `403`s `create_saved_query` / `run_live_query`). Per Fleet's RBAC, `create_saved_query` needs **admin / maintainer / gitops**, and `run_live_query` needs **admin / maintainer / technician / observer_plus** — so a single **maintainer** role covers both write tools and **admin is not required**. Recommended: an API-only user with the **lowest role** your tools need — **observer** for read-only deployments, **maintainer** for deployments that use the write tools. The MCP doesn't infer or log these rights — it only enforces the API-only requirement above.
