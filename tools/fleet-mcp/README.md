...
- **Read vs write mode = the token's Fleet role.** The MCP acts with exactly the role of `FLEET_API_KEY` (it has no mode of its own): an **observer** token → read-only mode (Fleet 403s `create_saved_query` / `run_live_query`); a **maintainer/admin** token → write mode. The startup `/me` check logs the detected identity + mode and **warns** if the token is write-capable — accounting for **global *and* per-fleet roles**, so a fleet admin/maintainer is flagged too. Recommended: an API-only user with the **lowest role** your tools need.
