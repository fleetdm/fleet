...
def gcal_service():
    ...
    if creds_json:
        ...
        sys.exit(
            "ERROR: set GCAL_SERVICE_ACCOUNT_JSON (raw JSON) or "
...
def desired_develop(current_start: dt.date, m: Milestone) -> tuple[dt.date, dt.date]:
    """Develop end = milestone due - 14 days. Start preserved from existing event."""
...
def plan_actions(
    milestones: list[Milestone], events: list[CalEvent]
) -> list[Action]:
    actions: list[Action] = []
    ...
    for m in milestones:
        ...
        if not m.out_of_band and m.number not in matched_milestone_ids["develop"]:
            # Develop create: we don't know a sensible start without history; skip with a warning.
            actions.append(
                Action(
                    kind="noop",
                    description=(
                        f"  ! No Develop event found for milestone {m.title} (due {m.due}). "
                        "Won't auto-create — add manually if missing."
                    ),
                )
            )
...
def main() -> int:
    ...
    actions = plan_actions(milestones, events)
