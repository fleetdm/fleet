...
| policy_id               | integer | query | **Required.** The ID of the policy to filter hosts by. `policy_response` must also be specified with `policy_id`.  If `policy_id` is specified, `status` and `team_id` cannot be specified.                                                                                                                                                 |
| policy_response         | string  | query | **Required.** Valid options are `passing` or `failing`.  `policy_id` must also be specified with `policy_response`. If `policy_response` is specified, `status` and `team_id` cannot be specified.                                                                                                                                          |
