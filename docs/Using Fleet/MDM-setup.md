...
The SCEP certificate/key pair generated by Fleet expires every 10 years. 

It's not recommended to change these unless your SCEP certificate or key was compromised.

This is because Fleet uses the pair to encrypt and decrypt macOS hosts' disk encryption (FileVault) keys. 

If the pair changes, the disk encryption key for all macOS hosts needs to be reset by the end user. The keys will no longer be viewable on each hosts' respective **Host details** page.
...
2. Turn disk encryption off and back on again. This will present the end user with instructions to reset their disk encryption key on their **My device** page. Learn how to turn disk encryption on/off [here](./MDM-disk-encryption.md#enforce-disk-encryption).
