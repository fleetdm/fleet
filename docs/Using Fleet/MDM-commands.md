...
* NotNow: the host responded wth "NotNow" status via the MDM protocol: the host received the command, but couldn’t execute it. The host will try to run the commadn the next time it comes online.
