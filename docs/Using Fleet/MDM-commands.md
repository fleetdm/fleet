...
* Pending: the command has yet to run on the host. The host either responded with "Idle" (no status) or "NotNow" (the host received the command but couldn't execute it) status via the MDM protocol.
