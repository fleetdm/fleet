...
- Does the certificate verify with `curl`? Try `curl -v -X POST https://fleetserver:port/api/latest/osquery/enroll`.
...
If you would like to manage hosts that can travel outside your VPN or intranet we recommend only exposing the "/api/latest/osquery" endpoint to the public internet.
