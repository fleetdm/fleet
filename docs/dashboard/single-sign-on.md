...
Fleet supports SAML single sign on capability. This feature is convenient for users and offloads responsibility for user authentication to a third party identity provider such as Salesforce or Onelogin. Fleet supports the SAML Web Browser SSO Profile using the HTTP Redirect Binding.
...
will use to send security assertions to Fleet. The value that you supply will be a fully qualified URL
...
based log in so that there is a 'back door' to log into Fleet and modify the SSO
