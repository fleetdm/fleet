...
- `install_software_in_parallel` specifies whether setup experience software is installed all at once. When `false`, Fleet installs it one item at a time, in alphabetical order by display name, and each item waits for the previous one to finish. When `true`, every item starts on the same agent check-in and the host downloads and installs them at the same time. The setup script still runs after all software is done. Applies to macOS, Windows, and Linux (default: `false`).
