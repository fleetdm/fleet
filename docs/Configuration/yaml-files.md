...
> When changing a fleet's name, you must first change it in the UI and then update your YAML. If you only update your YAML, the fleet will be deleted and the fleet's hosts will lose their settings. This happens because the hosts are transferred to "Unassigned".
...
reports:
agent_options:
controls: # Can be defined in fleets/no-fleet.yml too.
software: # Can be defined in fleets/no-fleet.yml too
org_settings: # Only default.yml
settings: # Required in fleets/fleet-name.yml, but can be defined in fleets/no-fleet.yml, where it is limited to webhook_settings
```
...
Specialized API-only users, who can modify configurations via GitOps but cannot access the Fleet UI, can be created through `fleetctl user create` with the `--api-only` flag. Best practice is to assign these users the `GitOps` role and specify global or fleet scope in the UI.

## labels

Labels can be specified in your `default.yml` and `fleets/fleet-name.yml` files using inline configuration or references to separate files in your `lib/` folder. Labels cannot be specified in `fleets/no-fleet.yml`.
...
Policies can be specified inline in your `default.yml`, `fleets/fleet-name.yml`, or `fleets/no-fleet.yml` files. They can also be specified in separate files in your `lib/` folder.
...
`default.yml`, `fleets/fleet-name.yml`, or `fleets/no-fleet.yml`
...
`default.yml` (for policies that neither install software nor run scripts), `fleets/fleet-name.yml`, or `fleets/no-fleet.yml`
...
> Currently, the `run_script` and `install_software` policy automations can only be configured for a fleet (`fleets/fleet-name.yml`) or "Unassigned" (`fleets/no-fleet.yml`). The automations can only be added to policies in which the script (or software) is defined in the same fleet (or "Unassigned"). `calendar_events_enabled` can only be configured for policies on a fleet.
...
Note that the `fleet_id` option isn't supported in GitOps.
...
`fleets/fleet-name.yml`, or `fleets/no-fleet.yml`
...
Can only be configured for all fleets (`org_settings`) and custom fleets (`settings`).
...
- `discard_reports_data` disables reports data and deletes existing reports. (default: `false`)
...
- `ios_fleet` is the fleet where iOS hosts are automatically added when they appear in Apple Business Manager. If not specified, defaults to "Unassigned".
...
reports referencing the rules to the desired fleets.
