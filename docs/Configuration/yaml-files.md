...
- `create_local_idp_user_account` specifices whether or not to automatically create a local idp user account on the host. Requires `enable_end_user_authentication` to be `true` and must be accompanied by `local_idp_user_account_configuration` object. Applies to macOS only. (default: `false`)
