...
- `windows_entra_tenant_ids` specifies list of Microsoft Entra tenant IDs to enable automatic (Autopilot) and manual enrollment by end user (**Settings > Accounts > Access work or school** on Windows). (default: none). Can only be configured for all teams (`default.yml`). Find your **Tenant ID**, on [Microsoft Entra ID > Home](https://entra.microsoft.com/#home).
