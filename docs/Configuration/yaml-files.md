...
- `alternate_sso_url` is an optional URL to use for SSO callbacks when different from the server URL. This is useful when you have separate URLs for admin access vs agent/API access (default: `""`).
...
org_settings:
  sso_settings:
    ...
    alternate_sso_url: https://admin.example.com # Optional, for dual URL setups
```
