...
- `microsoft_graph_credentials` is a list of Microsoft Graph API client credentials used to authenticate requests to the Microsoft Graph API. Fleet uses these to sync Windows Autopilot-registered devices so they appear as pending hosts before they enroll.
  Each entry contains:
  - `tenant_id` is the Microsoft Entra tenant ID. Must match one of the
  `windows_entra_tenant_ids` above (default: `""`).
  - `client_id` is the application (client) ID of the Entra app registration
  (default: `""`).
  - `client_secret` is the client secret for the Entra app registration
  (default: `""`).
...
controls:
  ...
  microsoft_graph_credentials: # Available in Fleet Premium
