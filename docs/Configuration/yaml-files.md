...
- `sso_url` is an optional URL to use for SSO authentication. When set, SSO will only work from this URL. This is useful for organizations with separate URLs for admin access vs agent/API access (default: `""`).
...
org_settings:
  sso_settings:
    ...
    sso_url: https://admin.example.com # Optional, SSO will only work from this URL
```
