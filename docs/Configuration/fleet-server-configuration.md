...
The SCEP certificate/key pair [generated by Fleet](../Using%20Fleet/MDM-setup.md#step-1-generate-the-required-files) expires every 10 years. It's not recommended to change these unless they were compromised. 

If your certificate/key pair was compromised and you need to change the pair, the disk encryption keys will no longer be viewable on all macOS hosts' **Host details** page until the keys are reset by the end user:

1. Regenerate the certificate and key and update the environment variables.

2. Turn disk encryption off and back on again. This will present the end user with instructions to reset their disk encryption key on their **My device** page.
