...
| software_status      | string | query |  Filters the hosts by the status of the managed software. Valid option is 'failed'. **Note: Filter must be used in combination with _software_title_id_. If this filter is used with non-managed software, the filter will be ignored.**   |
