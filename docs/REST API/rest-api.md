...
<!-- Return the following error if the user attempts to add a package but Fleet can't extract the version: Couldn't add. Fleet couldn't read the version from <package-name>. -->
...
```json
 {
   ...
   "detail": "Failed to verify: The install script finished with exit code 0 but the software is now missing. Fleet already tried to reinstall once.",
