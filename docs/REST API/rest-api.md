...
| platform          | string  | body | Comma-separated target platforms, currently supported values are "windows", "linux", "darwin". The default, an empty string means target all platforms. |
| critical          | boolean | body | _Available in Fleet Premium_. Mark policy as critical/high impact.                                                                                     |
| type | string | body | The type of the policy. Currently available `dynamic` and `patch`. Dynamic policies are custom policies with a editable query. Patch policies are policies that are tied to `patch_software_title_id` and query is automatically updated to include the newest Fleet-maintained app version. If not specified defaults to `dynamic` |
