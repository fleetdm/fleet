...
| populate_software     | boolean | query | If `true`, the response will include a list of installed software for each host, including vulnerability data. **This fetches a significant amount of data per host, causing significant CPU and RAM usage, so pay attention to page size and request concurrency when turning this on.** |
