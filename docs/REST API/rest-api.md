...
| enable_managed_local_account     | boolean | _Available in Fleet Premium._ During Setup Assistant, a managed local admin account will be created on macOS hosts that belong to no team if set to true. |
| end_user_local_account_type     | string | _Available in Fleet Premium._ Specifies the type of local end user account is created. (Default: `"admin"`) `enable_managed_local_account` must be true. |
