...
The `agent_options` field is only returned for global admins. The `sso_settings`, `smtp_settings`, and `email` fields are only returned for global admins and fleet admins (see the [Role-based access guide](https://fleetdm.com/guides/role-based-access)).
