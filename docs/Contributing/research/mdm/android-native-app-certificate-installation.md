...
1. MDM server must assign `CERT_INSTALL` delegated scope to the app via AMAPI policy
    ...
    ```
2. App then checks if the `DELEGATION_CERT_INSTALL` is granted.
https://github.com/marko-lisica/android-certificate-installer/blob/master/app/src/main/java/com/lisica/certinstaller/MainActivity.kt#L94

3. When delgated scope is granted, app can utulize `DevicePolicyManager.installKeyPair` to install certificate to Android host.
- Each installed certificate has an `alias` which is unique name that identifies certificate. This is needed for example to remove certificate using `removeKeyPair`
