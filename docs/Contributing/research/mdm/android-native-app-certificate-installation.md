...
1. MDM server must assign `CERT_INSTALL` delegated scope to the app via AMAPI policy
    ...
    ```
2. App then checks if the `DELEGATION_CERT_INSTALL` is granted. See more here: https://github.com/marko-lisica/android-certificate-installer/blob/master/app/src/main/java/com/lisica/certinstaller/MainActivity.kt#L94
