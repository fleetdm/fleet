...
The Scheduled Queries architecture enables the configuration, distribution, and execution of queries on a schedule across a fleet of devices. It leverages osquery's scheduled report capabilities to execute SQL queries on devices and return results to the Fleet server.

## Key components

- **Report Configuration**: The definition of a report, including the SQL statement, schedule, and target devices.
- **Configuration Distribution**: The mechanism for distributing report configurations to devices.
- **Result Collection**: The process of collecting and processing report results.
- **Result Storage**: The storage of report results for analysis and alerting.
...
### 1 - Fleet user creates a scheduled report

```
Fleet User -> API Client (Frontend or Fleetctl) -> Server -> DB
```

1. Fleet user creates a scheduled report for a fleet or globally through the UI or API.
2. Server stores the report configuration in the database.
...
2. Server merges fleet and global configurations.
...
1. osquery agent runs the report according to the schedule.
...
2. **Logger TLS**: The frequency of sending results (typically 10 seconds).
...
- **Report Complexity**: Complex report can consume significant CPU resources on devices.
