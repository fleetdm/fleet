...
Generated when a user turns on "Create local account from IdP" for macOS hosts that automatically enroll to a team (or no team). This enables Platform SSO local account creation with IdP credentials during macOS setup experience.
