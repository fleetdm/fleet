...
- **Relaxed poll**: once a host is known to be wakeable, the management session sets the DMClient `Poll` interval to a long value (8 hours). The change is reconciled against stored state, so it is sent once rather than on every session.
