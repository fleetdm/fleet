...
- **osquery**: The open-source agent that runs on devices and executes reports.
...
Fleet supports two main types of reports:
...
Live reports are executed in real-time and return results immediately. The process for executing a live report is as follows:
...
Scheduled reports are executed on a schedule and store results for later analysis. The process for executing a scheduled report is as follows:
