...
- `FLEET_JIT_USER_ROLE_FLEET_<FLEET_ID>`: Specifies team role for team with ID `<FLEET_ID>` to use when creating the user.
...
NOTE: Setting both `FLEET_JIT_USER_ROLE_GLOBAL` and `FLEET_JIT_USER_ROLE_FLEET_<FLEET_ID>` will cause an error during login as Fleet users cannot be Global users and belong to teams.
