...
+ **Changes to default values**: Fleet occasionally changes defaults to improve stability or usability. These changes are made thoughtfully and often come from customer feedback (e.g. default session expiration is too short, need to increase the window) or as a bug fix (e.g. default pagination causes a timeout, need to decrease the default per-page). Any changes to defaults are weighed carefully against the possibility of breaking existing workflows.
