...
- **WHEN** host one enrolled while the default was "Fleet A", the admin changes the default to "Fleet B", and host two then enrolls automatically
- **THEN** host two is assigned to "Fleet B" and host one remains in "Fleet A"

#### Scenario: Existing host keeps its fleet on re-enrollment
- **WHEN** a host already assigned to "Fleet C" (manually or via enroll secret) wipes and re-enrolls via Autopilot while the default fleet is "Fleet A"
- **THEN** the host remains in "Fleet C"
