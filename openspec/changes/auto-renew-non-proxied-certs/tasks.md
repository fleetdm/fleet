...
- [ ] 7b.2 Apply the same Subject CN/OU requirement as ACME: accept either `$FLEET_VAR_CERTIFICATE_RENEWAL_ID` (preferred) or the legacy `$FLEET_VAR_SCEP_RENEWAL_ID` per design.md Decision 2.7. Reject with `fleet.NewInvalidArgumentError`; the error message names only the preferred variable.
...
- [ ] 7b.4 Tests in `apple_mdm_test.go` covering: raw SCEP with new marker → accepted; with legacy marker → accepted (back-compat); without any marker → rejected; profile that also uses Fleet proxy vars → falls through to the existing per-CA validator path (no double-validation, no behavioral regression).
