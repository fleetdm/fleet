...
- [x] 1.1.1 Migration: add `host_certificates.origin` column (`enum('osquery','mdm')`, default `'osquery'` for existing rows) — `server/datastore/mysql/migrations/tables/20260428151210_AddOriginToHostCertificates.go` + test file
...
- [ ] 2.1 In `server/service/apple_mdm.go` `HandleHostMDMProfileInstallResult` path (or equivalent), detect when an acked profile contained a `com.apple.security.acme` payload on a macOS host
- [ ] 2.2 Enqueue `CertificateList` for the host using existing commander at `server/mdm/apple/commander.go:510`; reuse `RefetchCertsCommandUUIDPrefix` for tracking
- [ ] 2.3 Ensure `handleRefetchCertsResults` at `server/service/apple_mdm.go:4883` continues to process the response and call `UpdateHostCertificates` with `origin='mdm'`
- [ ] 2.4 Tests: simulate an ACME `InstallProfile` ack on a macOS host and verify `CertificateList` is queued; verify result handler ingests certs with `origin='mdm'`
- [ ] 2.5 Verify iOS/iPadOS path (`server/mdm/apple/apple_mdm.go:1585-1605`) still works unchanged — no regressions
...
- [ ] 6.2 Extend `UpdateHostCertificates` in `server/datastore/mysql/host_certificates.go:30-200` to scan newly inserted certs for the marker and INSERT missing `host_mdm_managed_certificates` rows
