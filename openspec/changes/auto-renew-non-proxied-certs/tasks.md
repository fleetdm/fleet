...
- [ ] 7b.2 Apply the same Subject CN/OU requirement as ACME: accept ONLY `$FLEET_VAR_CERTIFICATE_RENEWAL_ID` per design.md Decision 2.7 — raw-SCEP renewal-ID enforcement is net-new to this validator, so no back-compat against the legacy `$FLEET_VAR_SCEP_RENEWAL_ID` is owed. Reject with `fleet.NewInvalidArgumentError`; the error message names only the preferred variable.
...
- [ ] 7b.4 Tests in `apple_mdm_test.go` covering: raw SCEP with new marker → accepted; with legacy marker → rejected (preferred-only on this net-new surface); without any marker → rejected; profile that also uses Fleet proxy vars → falls through to the existing per-CA validator path (no double-validation, no behavioral regression).
