...
- [ ] 1.1 In `server/datastore/mysql/host_certificates.go`, add a top-level `hmmcBackfillGrace` duration constant (default `4 * time.Hour`) next to `hostCertificateAllowedOrderKeys`. Include a comment explaining its role: the in-flight grace window after which a NULL hmmc row is considered stuck and eligible for wide-pool recovery.
- [ ] 1.2 In the diff loop at lines 88-116, build a `toInsertBySHA1 map[string]*fleet.HostCertificateRecord` alongside `toInsert`. Populate in the same `else` branch that appends to `toInsert`.
- [ ] 1.3 Replace the matcher block at lines 115-152 with the new shape:
...
- [ ] 1.4 Verify the `MDMManagedCertificate` struct (in `server/fleet/apple_mdm.go`) exposes the field needed for the stuck check (`UpdatedAt time.Time`), and that `ListHostMDMManagedCertificates`'s SELECT loads it. If `UpdatedAt` is missing, add it to the struct's `db` tags and the SELECT column list. Ensure no breaking change to other consumers.
- [ ] 1.5 Add an inline comment on the pool-selection block referencing the in-flight vs stuck distinction, calling out that the steady-state path matches today's behavior.

## 2. Tests

- [ ] 2.1 In `server/datastore/mysql/host_certificates_test.go`, add a top-level entry `{"Matcher recovers stuck hmmc rows", testMatcherRecoversStuckHMMCRows}` to the `TestHostCertificates` cases slice.
- [ ] 2.2 Implement `testMatcherRecoversStuckHMMCRows(t *testing.T, ds *Datastore)` with subtests covering:
...
- [ ] 2.3 Reuse `BulkUpsertMDMAppleHostProfiles` / `BulkUpsertMDMWindowsHostProfiles` for profile state setup, and the existing `generateTestHostCertificateRecord` helper for cert payloads. Backdate `hmmc.updated_at` via raw SQL.

## 3. Verification

- [ ] 3.1 `go build ./...` clean.
- [ ] 3.2 `make lint-go-incremental` reports 0 issues against changes since `origin/main`.
- [ ] 3.3 `MYSQL_TEST=1 REDIS_TEST=1 FLEET_MYSQL_TEST_PORT=3309 go test ./server/datastore/mysql/ -count=1 -run "TestHostCertificates|TestMDMApple/MDMManagedSCEPCertificates" -timeout 5m` passes.
...
- [ ] 4.1 Add a `changes/<issue>-scep-matcher-recovery` (or similar) one-line entry. Suggested wording: `Recover SCEP-managed certificate state when an osquery cert ingest previously missed linking the renewed cert.`
