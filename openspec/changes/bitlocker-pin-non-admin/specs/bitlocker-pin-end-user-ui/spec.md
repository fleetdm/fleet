...
On the My device page for a Windows host with `disk_encryption.action_required = "create_pin"` and `fleetd_can_set_pin = true`, the **Create PIN** action SHALL open a modal titled "Create PIN" with the intro "Set a BitLocker PIN to protect your data if this host is lost or stolen. You'll need to enter it each time your host starts up.", a masked "BitLocker PIN" field with helper text "Must be 6–20 digits. Keep it somewhere safe. Fleet can't show this PIN to you or your IT team later.", a masked "Confirm PIN" field, a "Cancel" button and a primary "Save" button. The helper text SHALL NOT claim that Fleet does not store the PIN: under the relay design the server holds it encrypted until the agent collects it, so the Figma's original wording ("This PIN isn't saved by Fleet or your IT team") would be inaccurate security copy. The replacement above states what is true, that nobody can read the PIN back, and is pending product sign-off. Save SHALL be disabled until both fields contain the same 6 to 20 ASCII digits.
