...
func TestEvalMigrationStatus(t *testing.T) {
	for _, tc := range []struct {
		...
		allowMissing bool
		wantExit     bool
		wantOut      string // optional substring expected on stdout
	}{
		{
		...
		{
			name:     "unknown migrations tolerated outside dev",
			status:   &fleet.MigrationStatus{StatusCode: fleet.UnknownMigrations, UnknownTable: []int64{1}},
			devMode:  false,
			wantExit: false,
		},
		...
		{
			name:         "needs v4732 fix tolerated when missing allowed",
			status:       &fleet.MigrationStatus{StatusCode: fleet.NeedsFleetv4732Fix},
			allowMissing: true,
			wantExit:     false,
		},
		...
		{
			name:         "unknown v4732 state tolerated when missing allowed",
			status:       &fleet.MigrationStatus{StatusCode: fleet.UnknownFleetv4732State},
			allowMissing: true,
			wantExit:     false,
		},
		...
		{
			name:         "some migrations completed tolerated when missing allowed",
			status:       &fleet.MigrationStatus{StatusCode: fleet.SomeMigrationsCompleted, MissingTable: []int64{7}},
			allowMissing: true,
			wantExit:     false,
		},
