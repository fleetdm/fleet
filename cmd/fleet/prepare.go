...
`,
	...
	// TODO(lucas): We'll always use one DEP name, "fleet". Pause and ponder.
	const mdmAppleDEPName = "fleet"

	mdmAppleSetupCmd := &cobra.Command{
		Use:   "setup",
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			err = mdmAppleDEPStorage.StoreTokenPKI(cmd.Context(), mdmAppleDEPName, pemCert, pemKey)
			if err != nil {
	...
	mdmAppleDEPPushTokenCmd := &cobra.Command{
		Use:   "dep-auth-token",
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			depPEMCert, depPEMKey, err := depStorage.RetrieveTokenPKI(cmd.Context(), mdmAppleDEPName)
			if err != nil {
			...
			if err := depStorage.StoreAuthTokens(cmd.Context(), mdmAppleDEPName, tokens); err != nil {
			...
			defineProfileRequest, err := client.NewRequestWithContext(
				cmd.Context(), mdmAppleDEPName, depStorage,
			...
			if err := depStorage.StoreAssignerProfile(
				cmd.Context(), mdmAppleDEPName, defineProfileResponse.ProfileUUID,
			); err != nil {
