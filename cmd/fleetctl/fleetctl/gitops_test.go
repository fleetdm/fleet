...
`,
	...
	assert.Contains(t, err.Error(), "consider renaming the file")
	t.Setenv("TEST_TEAM_NAME", "no TEam")
	_, err = RunAppNoChecks([]string{"gitops", "-f", tmpFile.Name()})
	require.Error(t, err)
	assert.Contains(t, err.Error(), fmt.Sprintf("file %q for No Team must be named 'no-team.yml'", tmpFile.Name()))
	...
	_, err = RunAppNoChecks([]string{"gitops", "-f", tmpFile.Name()})
	require.Error(t, err)
	assert.Contains(t, err.Error(), fmt.Sprintf("file %q for unassigned hosts must be named 'unassigned.yml'", tmpFile.Name()))
	t.Setenv("TEST_TEAM_NAME", "unAsSIgNeD")
	_, err = RunAppNoChecks([]string{"gitops", "-f", tmpFile.Name()})
	require.Error(t, err)
	assert.Contains(t, err.Error(), fmt.Sprintf("file %q for unassigned hosts must be named 'unassigned.yml'", tmpFile.Name()))
