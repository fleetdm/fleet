...
// TestGitOpsTeamLabelOrderingNoLabelChanges is the regression guard for
// issue #44046: when a fleet GitOps apply touches profiles that reference
// pre-existing labels (no `labels:` block change in this YAML), the apply
// must still succeed.
func TestGitOpsTeamLabelOrderingNoLabelChanges(t *testing.T) {
	license := &fleet.LicenseInfo{Tier: fleet.TierPremium, Expiration: time.Now().Add(24 * time.Hour)}
	...
	// The label already exists in the database. The YAML doesn't declare any
	// labels block, so doGitOpsLabels should be skipped entirely.
	labelApplyCallCount := 0
	ds.ApplyLabelSpecsWithAuthorFunc = func(ctx context.Context, specs []*fleet.LabelSpec, authorID *uint) error {
		labelApplyCallCount++
		return nil
	}
	ds.GetLabelSpecsFunc = func(ctx context.Context, filter fleet.TeamFilter) ([]*fleet.LabelSpec, error) {
		return []*fleet.LabelSpec{
			{Name: "preexisting-label", LabelType: fleet.LabelTypeRegular, LabelMembershipType: fleet.LabelMembershipTypeManual},
		}, nil
	}
	ds.LabelIDsByNameFunc = func(ctx context.Context, names []string, filter fleet.TeamFilter) (map[string]uint, error) {
		out := map[string]uint{}
	...
	ds.LabelsByNameFunc = func(ctx context.Context, names []string, filter fleet.TeamFilter) (map[string]*fleet.Label, error) {
		out := map[string]*fleet.Label{}
		for i, name := range names {
			out[name] = &fleet.Label{ID: uint(50 + i), Name: name}
		}
		return out, nil
	}
	...
	teamYAMLPath := filepath.Join(tmpDir, "team.yml")
	require.NoError(t, os.WriteFile(teamYAMLPath, fmt.Appendf(nil, `
name: "Test No Label Changes Team"
team_settings:
  secrets:
    - secret: "ABC"
queries:
...
`, mobileConfigPath), 0o600))
	...
	_, err = RunAppNoChecks([]string{"gitops", "-f", teamYAMLPath})
	require.NoError(t, err, "gitops should succeed when profiles reference pre-existing labels (no `labels:` block change)")
	require.NotNil(t, savedTeam)
	assert.Zero(t, labelApplyCallCount, "ApplyLabelSpecsWithAuthor should not be called when YAML has no labels block")
}
