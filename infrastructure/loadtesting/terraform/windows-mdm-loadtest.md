...
- Host x profile fan-out. Each assigned profile creates one row per host in `windows_mdm_command_queue` and one in `host_mdm_windows_profiles`. 100 profiles x 100k hosts is ~10M rows in each table on a full re-push. Note the row count is per profile, not per LocURI: a single profile becomes one command row per host regardless of how many settings it carries.
...
- Disk encryption escrow. Enabling encryption fans out the `EnforceBitLockerEncryption` orbit notification to all hosts and produces a one-time burst of recovery-key writes into `host_disk_encryption_keys` (plus archive and activity rows). This axis is currently NOT testable.
