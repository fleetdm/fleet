...
* Implemented the `executable_hashes` `fleetd` table, providing easy access to the sha256 hash of and absolute path to the main executable of (a) mac app bundle(s) matching the `path` provided in the query's WHERE clause. Can match a specific bundle via equality or multiple bundles via LIKE.
