...
func NewUpdater(opt Options) (*Updater, error) {
	...
	if err := migrateTUFMetadata(&opt); err != nil {
		log.Error().Err(err).Msg("failed to migrate to new TUF repository")
	}
...
func migrateTUFMetadata(opt *Options) (err error) {
	...
	defer func() {
		// If migration was successful or already migrated then use the new repository URL.
		if err == nil {
	...
	//
	// Perform migration to new TUF repository by removing the current
