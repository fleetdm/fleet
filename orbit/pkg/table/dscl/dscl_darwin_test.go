...
func TestParseDSCLOutput(t *testing.T) {
	...
	value, err := parseDSCLOutput([]byte(noKeySample))
	require.NoError(t, err)
	require.Nil(t, value)

	const keySample0 = `PrimaryGroupID: 20`
	value, err = parseDSCLOutput([]byte(keySample0))
	require.NoError(t, err)
 ...
 /Library/User Pictures/Animals/Penguin.tif`
	value, err = parseDSCLOutput([]byte(keySample1))
	require.NoError(t, err)
	require.NotNil(t, value)
	...
	value, err = parseDSCLOutput([]byte(keySample2))
	require.NoError(t, err)
	require.NotNil(t, value)
...
<dict>
	...
	value, err = parseDSCLOutput([]byte(keySample3))
	require.NoError(t, err)
	require.NotNil(t, value)
...
<dict>
	...
	value, err = parseDSCLOutput([]byte(keySample4))
	require.NoError(t, err)
	require.NotNil(t, value)
 ...
 BUILTIN\Local System`
	value, err = parseDSCLOutput([]byte(keySample5))
	require.NoError(t, err)
	require.NotNil(t, value)
 ...
 BUILTIN\Local System`, *value)
	...
	value, err = parseDSCLOutput([]byte(keySample6))
	require.NoError(t, err)
	require.NotNil(t, value)
