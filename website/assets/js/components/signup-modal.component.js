...
parasails.registerComponent('signupModal', {
  ...
  <transition name="modal" v-on:leave="leave" v-bind:css="false">
    <div class="modal fade" id="signupmodal" tabindex="-1" role="dialog" @click="$emit('close')">
      ...
      <div class="modal-dialog custom-width position-relative" role="document" purpose="modal-dialog" >
        <div class="modal-content" purpose="modal-content" v-on:click.stop>
          <!-- <button type="button" class="position-absolute" data-dismiss="modal" aria-label="Close" purpose="modal-close-button" @click="$emit('close')">&times;</button> -->
          <slot></slot>

  <!-- <modal id="signupmodal" class="modal fade" hide-close-button="true" aria-hidden="true" > -->
    <div purpose="signup-modal">
    <h3>Welcome to Fleet</h3>
    <p>We just need a few details in order to get started.</p>
    ...
    <ajax-form action="signup" purpose="modal-form" class="self-service-register" :syncing.sync="signupSyncing" :cloud-error.sync="cloudError" :form-errors.sync="signupFormErrors" :form-data="signupFormData" :form-rules="signupFormRules" v-if="formToDisplay === 'signup'">
      ...
      <ajax-button tabindex="6" purpose="submit-button" spinner="true" type="submit" :syncing="signupSyncing" class="btn btn-block btn-lg btn-primary mt-4" v-if="!cloudError">Agree and continue</ajax-button>
      <ajax-button tabindex="7" purpose="submit-button" type="button" :syncing="signupSyncing" class="btn btn-block btn-lg btn-primary mt-4" v-if="cloudError" @click="clickResetForm()">Try again</ajax-button>
    </ajax-form>
    <ajax-form class="w-100" action="login" purpose="modal-form"  :syncing.sync="signupSyncing" :cloud-error.sync="cloudError" :form-data="loginFormData" :form-rules="loginFormRules" :form-errors.sync="loginFormErrors" @submitted="submittedLoginForm()" v-else>
      <div class="form-group">
      ...
      <div class="pb-3">
        <ajax-button tabindex="3" :syncing="signupSyncing" spinner="true" purpose="submit-button" class="btn-primary mt-4 btn-lg btn-block">Sign in</ajax-button>
      </div>
    </ajax-form>
    </div>
