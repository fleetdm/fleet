module.exports = {
  ...
  fn: async function () {
    ...
    // Fetch projects
    let projects = await sails.helpers.http.get(`https://api.github.com/orgs/fleetdm/projects`, {}, baseHeaders).retry('2');// let projects = [];// « hack if you get rate limited and want to test beta projets
    ...
    await sails.helpers.flow.simultaneouslyForEach(projects, async(project)=>{
      ...
      if (!project._isBetaProject) {
        columns = await sails.helpers.http.get(`https://api.github.com/projects/${project.id}/columns`, {}, baseHeaders).retry('2');
      } else {
      ...
      await sails.helpers.flow.simultaneouslyForEach(columns, async(column)=>{
        ...
        let cards;
        if (!project._isBetaProject) {
          cards = await sails.helpers.http.get(`https://api.github.com/projects/columns/${column.id}/cards`, {}, baseHeaders).retry('2');
        } else {
        ...
        await sails.helpers.flow.simultaneouslyForEach(cards, async(card)=>{
          ...
          if (!project._isBetaProject) {
            if (!card.content_url) {
              ...
              let issue = await sails.helpers.http.get(card.content_url, {}, baseHeaders).retry('2');
              labels = issue.labels;
            }
          } else {
            labels = card.labels;
