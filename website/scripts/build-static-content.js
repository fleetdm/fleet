                ...
                if(embeddedMetadata.articleImageUrl) {
                  ...
                  if (isURL) {
                    ...
                    if(!embeddedMetadata.articleImageUrl.match(/\d+x\d+@2x.+/)){
                      throw new Error(`Failed compiling markdown content. An article page has a articleImageUrl value that does not follow the website's image naming conventions. Please update the image's filename and articleImageUrl value in "${path.join(topLvlRepoPath, pageSourcePath)}" to include the CSS dimensions of the image (pixel dimensions * 0.5 postfixed with "@2x" e.g., "foo-200x300@2x.png")  and try running this script again.`);
                    }
