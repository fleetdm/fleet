  ...
  fn: async function () {
    ...
    for(let osquerySchemaTable of rawOsqueryTables) {
      ...
      if(!fleetOverridesForTable) {
        ...
        // Since we don't have a Fleet override for this table, we'll set the url for this table in the merged schema to be a link to create the Fleet override table YAML.
        ...
        expandedTableToPush.url = 'https://github.com/fleetdm/fleet/new/main/schema/tables/?filename='+encodeURIComponent(expandedTableToPush.name)+'.yml&value='+encodeURIComponent(sampleYamlSchemaForThisTable);
    ...
    for (let fleetOverridesForTable of fleetOverridesForTables) {
      if(!fleetOverridesForTable.name){
        throw new Error(`A table in the Fleet overrides schema is missing a 'name' (${JSON.stringify(fleetOverridesForTable)}). To resolve, make sure this table has a "name" property.`);
      }
      ...
      if(!fleetSchemaTableExistsInOsquerySchema) { // If a table in the Fleet schema does not exist in the osquery schema, we'll add it to the final schema after making sure that it has the required values.
        if(!fleetOverridesForTable.description) {
          throw new Error(`Could not add a new table from the Fleet overrides to final merged schema, the "${fleetOverridesForTable.name}" table is missing a 'description' value. To resolve, add a description to this table to the Fleet overrides schema at ${path.resolve(topLvlRepoPath+'/schema/tables', fleetOverridesForTable.name+'.yml')} .\n Tip: If this table is meant to override a table in the osquery schema, you may want to check that the "name" value of the added table is the same as the table in the osquery schema located at https://github.com/osquery/osquery-site/source/src/data/osquery_schema_versions/${VERSION_OF_OSQUERY_SCHEMA_TO_USE}.json`);
        }
        if(!fleetOverridesForTable.platforms) {
          throw new Error(`Could not add a new table from the Fleet overrides to final merged schema, the "${fleetOverridesForTable.name}" table is missing a 'platforms' value. To resolve, add an array of platforms to this table to the Fleet overrides schema at ${path.resolve(topLvlRepoPath+'/schema/tables', fleetOverridesForTable.name+'.yml')} .\n Tip: If this table is meant to override a table in the osquery schema, you may want to check that the "name" value of the added table is the same as the table in the osquery schema located at https://github.com/osquery/osquery-site/source/src/data/osquery_schema_versions/${VERSION_OF_OSQUERY_SCHEMA_TO_USE}.json`);
        }
        if(fleetOverridesForTable.evented === undefined) {
          throw new Error(`Could not add a new table from the Fleet overrides to final merged schema, the "${fleetOverridesForTable.name}" table is missing a 'evented' value. To resolve, add an evented value to this table to the Fleet overrides schema at ${path.resolve(topLvlRepoPath+'/schema/tables', fleetOverridesForTable.name+'.yml')} .\n Tip: If this table is meant to override a table in the osquery schema, you may want to check that the "name" value of the added table is the same as the table in the osquery schema https://github.com/osquery/osquery-site/source/src/data/osquery_schema_versions/${VERSION_OF_OSQUERY_SCHEMA_TO_USE}.json`);
        } else if(typeof fleetOverridesForTable.evented !== 'boolean') {
          throw new Error(`Could not add a new table from the Fleet overrides to the final merged schema. The "${fleetOverridesForTable.name}" table has an invalid "evented" value. (expected a boolean, but instead got a ${typeof fleetOverridesForTable.evented}) To resolve, change the "evented" value for this table to be true or false, then try running this script again.`)
        }
        if(!fleetOverridesForTable.columns) {
          throw new Error(`Could not add a new table from the Fleet overrides to final merged schema. The "${fleetOverridesForTable.name}" table is missing a "columns" value. To resolve, add an array of columns to this table to the Fleet overrides schema at ${path.resolve(topLvlRepoPath+'/schema/tables', fleetOverridesForTable.name+'.yml')} .\n Tip: If this table is meant to override a table in the osquery schema, you may want to check that the "name" value of the added table is the same as the table in the osquery schema located at https://github.com/osquery/osquery-site/source/src/data/osquery_schema_versions/${VERSION_OF_OSQUERY_SCHEMA_TO_USE}.json`);
        } else {

          for(let columnToValidate of fleetOverridesForTable.columns) { // Check each column in the table to make sure it has the required values, and that all values are the correct type.

            if(!columnToValidate.name) {
              throw new Error(`Could not add a new table from the Fleet overrides schema. A column in the "${fleetOverridesForTable.name}" table is missing a "name". To resolve, make sure every column in the table located at ${path.resolve(topLvlRepoPath+'/schema/tables', fleetOverridesForTable.name+'.yml')} has a "name" property.`);

            } else if(typeof columnToValidate.name !== 'string') {
              throw new Error(`Could not add a new table from the Fleet overrides schema. A column in the "${fleetOverridesForTable.name}" table located at /schema/tables/${fleetOverridesForTable.name}.yml has an invalid "name" (expected a string, but instead got ${typeof columnToValidate.name}).\nTo resolve, make sure that the "name" of all columns in this table are strings.`);
            }//•

            if(!columnToValidate.type) {
              throw new Error(`Could not add a new table from the Fleet overrides schema. The "${columnToValidate.name}" column of the "${fleetOverridesForTable.name}" table is missing a "type". To resolve add a "type" to the "${columnToValidate.name}" column at ${path.resolve(topLvlRepoPath+'/schema/tables', fleetOverridesForTable.name+'.yml')}.`);
            } else if(typeof columnToValidate.type !== 'string') {
              throw new Error(`Could not add a table from the Fleet overrides schema. The "type" of the "${columnToValidate.name}" column of the "${fleetOverridesForTable.name}" table at ${path.resolve(topLvlRepoPath+'/schema/tables', fleetOverridesForTable.name+'.yml')} has an invalid value. (expected a string, but got a ${typeof fleetOverrideColumn.type}) To resolve, change the value of the column's "type" be a string.`);
            }//•

            if(!columnToValidate.description) {
              throw new Error(`Could not add a new table from the Fleet overrides schema. The "${columnToValidate.name}" column of the "${fleetOverridesForTable.name}" table is missing a "description". To resolve add a "description" property to the "${columnToValidate.name}" column at ${path.resolve(topLvlRepoPath+'/schema/tables', fleetOverridesForTable.name+'.yml')}`);
            } else if (typeof columnToValidate.description !== 'string') {
                throw new Error(`Could not add a table from the Fleet overrides schema. The "description" property of the "${columnToValidate.name}" column of the "${fleetOverridesForTable.name}" table at ${path.resolve(topLvlRepoPath+'/schema/tables', fleetOverridesForTable.name+'.yml')} has an invalid "description" value. To resolve, change the "description" property of the added column to be a string.`);
            }//•

            if(columnToValidate.required === 'undefined') {
              throw new Error(`Could not add a new table from the Fleet overrides schema. The "${columnToValidate.name}" column of the "${fleetOverridesForTable.name}" table is missing a "required" property. To resolve add a "required" property to the "${columnToValidate.name}" column at ${path.resolve(topLvlRepoPath+'/schema/tables', fleetOverridesForTable.name+'.yml')}`);
            } else if (typeof columnToValidate.required !== 'boolean') {
              throw new Error(`Could not add a new table from the Fleet overrides schema. The "${columnToValidate.name}" column of the "${fleetOverridesForTable.name}" table has an invalid "required" value. (expected a boolean, but instead got a ${typeof columnToValidate.required}) To resolve, change the "required" property of the added column to be a boolean.`);
            }//•

            if(columnToValidate.platforms) {
              if(!_.isArray(columnToValidate.platforms)){
                throw new Error(`Could not add a new table from the Fleet overrides schema. The "platforms" property of the "${columnToValidate.name}" column of the "${fleetOverridesForTable.name}" table has an invalid value. To resolve, change the "platforms" of this column to an array`);
              }//•
            }
          }//∞ After each column in Fleet overrides table
        }
