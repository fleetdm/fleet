module.exports = {
  ...
  friendlyName: 'Deliver webinar registration request',
  ...
  exits: {
    success: {description: 'A user\'s webinar registration request was successfully submitted.'},
    invalidEmailDomain: {
  ...
  fn: async function ({firstName, lastName, emailAddress, webinarName}) {
    ...
    sails.helpers.flow.build(async ()=>{
      let recordIds = await sails.helpers.salesforce.updateOrCreateContactAndAccount.with({
        ...
        description: `Submitted a form to register for the ${webinarName} webinar.`,
      ...
      await sails.helpers.salesforce.createHistoricalEvent.with({
        ...
        intentSignal: 'Registered for webinar',
      ...
      if(err){
        sails.log.warn(`Background task failed: When a user (email: ${emailAddress} submitted a form to register for the ${webinarName} webinar, a Contact/Account/website activity record could not be created/updated in the CRM.`, require('util').inspect(err));
      }
      return;
    });//_∏_
