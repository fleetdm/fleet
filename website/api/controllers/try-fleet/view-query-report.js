module.exports = {
  ...
  fn: async function ({hostPlatform, tableName}) {
    ...
    if(!this.req.me){
      throw {redirect: `/try-fleet/login?targetPlatform=${hostPlatform}` };
    }
    ...
    .intercept((error)=>{
      return new Error(`When sending an API request to ${sails.config.custom.fleetBaseUrlForQueryReports}/api/v1/fleet/queries/${queryIdToGetReportFor}/report to get the latest query report for the ${tableName} table, an error occured: ${error}`);
    });
