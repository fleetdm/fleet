module.exports = {
  ...
  fn: async function ({naturalLanguageQuestion}) {
    ...
    let filteredTables = await sails.helpers.ai.prompt(schemaFiltrationPrompt, 'claude-sonnet-4-6-20260218', true, 'Please only respond in valid JSON with no codefences or backticks.')
    .intercept((err)=>{
    ...
    let sqlReport = await sails.helpers.ai.prompt.with({prompt:sqlPrompt, baseModel:'claude-sonnet-4-6-20260218', expectJson: true})
    .intercept((err)=>{
      if(this.req.isSocket){
        // If this request was from a socket and an error occurs, broadcast an 'error' event and unsubscribe the socket from this room.
        sails.sockets.broadcast(roomId, 'error', {error: err});
