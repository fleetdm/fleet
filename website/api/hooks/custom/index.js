...
---------------------------------------------------------------------${suffix}`);
      ...
      sails.after('hook:organics:loaded', async ()=>{
        if(sails.config.custom.eventbriteApiToken) {
          ...
          let futureGitopsEvents = await sails.helpers.http.get.with({
            ...
            sails.log.warn(`When the server started, a list of future gitiops workshops could not be obtained from the Eventbrite API. Full error: ${require('util').inspect(err)}`);
            return {
              events: [],
            };
          });
          ...
          await sails.helpers.flow.simultaneouslyForEach(eventsToGetDetailsFor, async (event)=>{
            ...
            // Create strings of the event's start and end times
            ...
            if(event.venue_id) {
              let eventVenueResponse = await sails.helpers.http.get.with({
                ...
                sails.log.warn(`When the server started, details about a venue for an event (${event.name.text}) could not be obtained from the Eventbrite API. Full error: ${require('util').inspect(err)}`);
            ...
            // Add the event to the list of futureGitopsWorkshops.
