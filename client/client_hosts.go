...
func (c *Client) GetHosts(query string) ([]fleet.HostResponse, error) {
	...
	var responseBody fleet.ListHostsResponse
...
func (c *Client) GetHost(id uint) (*fleet.HostDetailResponse, error) {
	...
	var responseBody fleet.GetHostResponse
...
func (c *Client) translateTransferHostsToIDs(hosts []string, label string, team string) ([]uint, uint, uint, error) {
	...
	var responseBody fleet.TranslatorResponse
	...
	params := fleet.TranslatorRequest{List: translatePayloads}
...
func (c *Client) TransferHosts(hosts []string, label string, status, searchQuery string, team string) error {
	...
	if len(hosts) != 0 {
		...
		var responseBody fleet.AddHostsToTeamResponse
		params := fleet.AddHostsToTeamRequest{TeamID: teamIDPtr, HostIDs: hostIDs}
	...
	var responseBody fleet.AddHostsToTeamByFilterResponse
	params := fleet.AddHostsToTeamByFilterRequest{
		TeamID:  teamIDPtr,
		Filters: &filter,
	}
