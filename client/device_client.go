...
func (dc *DeviceClient) getListDevicePolicies(token string) ([]*fleet.HostPolicy, error) {
	...
	var responseBody fleet.ListDevicePoliciesResponse
	err := dc.request(verb, path, token, "", nil, &responseBody)
	return responseBody.Policies, err
}

func (dc *DeviceClient) getMinDesktopPayload(token string) (fleet.FleetDesktopResponse, error) {
	verb, path := "GET", "/api/latest/fleet/device/%s/desktop"
	var r fleet.FleetDesktopResponse
	err := dc.request(verb, path, token, "", nil, &r)
	return r, err
}

func (dc *DeviceClient) DesktopSummary(token string) (*fleet.FleetDesktopResponse, error) {
	...
	if errors.Is(err, notFoundErr{}) {
		...
		return &fleet.FleetDesktopResponse{
...
func (dc *DeviceClient) ReportError(token string, fleetdErr fleet.FleetdError) error {
	...
	req := fleet.FleetdErrorRequest{FleetdError: fleetdErr}
	return retry.Do(
		func() error {
