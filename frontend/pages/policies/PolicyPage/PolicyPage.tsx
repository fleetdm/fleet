...
import { RUN_POLICY_STEPS } from "utilities/constants";
...
const PolicyPage = ({
  ...
  const [step, setStep] = useState(RUN_POLICY_STEPS[1]);
  ...
  const renderScreen = () => {
    const step1Opts = {
      ...
      goToSelectTargets: () => setStep(RUN_POLICY_STEPS[2]),
      onOpenSchemaSidebar,
    ...
    const step2Opts = {
      ...
      targetedTeams,
      targetsTotalCount,
      goToQueryEditor: () => setStep(RUN_POLICY_STEPS[1]),
      goToRunQuery: () => setStep(RUN_POLICY_STEPS[3]),
      setSelectedTargets,
      setTargetedHosts,
      setTargetedLabels,
      setTargetedTeams,
    ...
    switch (step) {
      case RUN_POLICY_STEPS[2]:
        return <SelectTargets {...step2Opts} />;
      case RUN_POLICY_STEPS[3]:
  ...
  const isFirstStep = step === RUN_POLICY_STEPS[1];
  const showSidebar =
    isFirstStep &&
    isSidebarOpen &&
