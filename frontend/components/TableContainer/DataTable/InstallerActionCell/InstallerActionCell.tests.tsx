...
describe("Issue cell", () => {
  it("renders add button if installer is available", async () => {
    render(<InstallerActionCell value="available" />);

    expect(screen.getByText(/add/i)).toBeInTheDocument();
  });
  it("renders --- if installer is unavailable", async () => {
    render(<InstallerActionCell value="unavailable" />);

    expect(screen.getByText(/---/i)).toBeInTheDocument();
  });
  it("renders checkmark if installer is already added", async () => {
    render(<InstallerActionCell value="added" />);

    const icon = screen.getByTestId("success");
