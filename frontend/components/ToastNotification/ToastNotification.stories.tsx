...
/**
 * Batch — one toast per failed item in a bulk operation (the
 * `renderMultiFlash` replacement). Each item supports the same `response`
 * option as `notify.error`, so every failure exposes its raw API response.
 */
export const Batch: Story = {
  render: () => (
    <>
      <ToastNotification />
      <TriggerButton
        label="Show batch (bulk update failures)"
        onClick={() =>
          notify.batch([
            {
              variant: "error",
              message: "Couldn't update Firefox. Please try again.",
              options: {
                response: {
                  status: 409,
                  statusText: "Conflict",
                  data: { error: "install already pending", software_id: 1 },
                },
              },
            },
            {
              variant: "error",
              message: "Couldn't update Slack. Please try again.",
              options: {
                response: {
                  status: 500,
                  statusText: "Internal Server Error",
                  data: { error: "installer not found", software_id: 2 },
                },
              },
            },
            {
