...
describe('Errors - reducer', () => {
  it(
    'Updates state with errors object when an action that has a payload with an errors object is dispatched',
    () => {
      const payload = {
        errors: {
  ...
  it(
    'Updates state by setting errors to null when the RESET_ERRORS action is dipatched',
    () => {
      const errorsState = {
        errors: {
          base: "inserting pack: Error 1136: Column count doesn't match value count at row 1",
