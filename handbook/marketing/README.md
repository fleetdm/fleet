...
Here are some links that may be helpful to share with public relation teams and press contacts:
- Read [about the company](https://fleetdm.com/handbook/company) or the [Series A funding announcement](https://techcrunch.com/2022/04/28/fleet-nabs-20m-to-enable-enterprises-to-manage-their-devices/).  (For a full list of investors and advisors, request access to ["🌧️🦉 Advisors & investors" (confidential Google doc)](https://docs.google.com/spreadsheets/d/15knBE2-PrQ1Ad-QcIk0mxCN-xFsATKK9hcifqrm0qFQ/edit#gid=1068113636).)
- Check out [logos, colors, and branding recommendations](https://fleetdm.com/logos) for Fleet's logo usage guidelines, plus some fun resources like mobile and desktop wallpapers.
- Why Fleet?
  - Fleet is the only complete device management product that is 100% open source.  [Why open source?](https://fleetdm.com/handbook/company/why-this-way#why-open-source). 
  - Above all, what makes Fleet different is openness.  Fleet is open, flexible, accessible, and clear.  Check out [concept art and inspiration from Q1 2023](https://drive.google.com/drive/folders/1fjwbl_XmrCi1HtehFFGdKA0AYhy9AIpC?usp=sharing) or for more context, request access to ["Crown jewels" (confidential Google doc)](https://docs.google.com/document/d/1E0VU4AcB6UTVRd4JKD45Saxh9Gz-mkO3LnGSTBDLEZo/edit#heading=h.uc23vwwe87n8).
  - Fleet is universal and cross-platform.  It works with macOS, ChromeOS, Windows, or Linux.  But beyond workstations, it also allows enrolling virtual servers in any cloud, containers, and even infrastructure in custom datacenters.
  - Fleet is engineer-friendly.  Anything you can do in the graphical user interface, you can also do in the REST API or `fleetctl` command-line tool.  System engineers, IT engineers, and security engineers love this, since it speeds up integrations, gives them more control, enables modern GitOps / "configuration as code" (CaC) use cases, and prevents their team from getting stuck behind the arbitrary limitations of legacy solutions like Airwatch.
  - Fleet is enterprise-friendly: the only open-source MDM designed and tested for large organizations with lots of computers.  That means it works great with (and has been battle-tested to support) tens of thousands of workstations and servers enrolled at the same time.  In fact, many production deployments of Fleet have hundreds of thousands of servers and workstations.  The largest Fleet deployment that the Fleet team is aware of has over 400,000 hosts enrolled.
