...
| Attachments                 | Whitelist (*Google's term, which should be updated to allow-list*) the following uncommon filetypes                      | Empty   |                                    |                                                                                                        |
...
We recommend using the Gmail web interface on computers and the Gmail app on mobile devices. The UI on the official clients includes security information that is not visible in standard mail clients. We do allow a few of them at the moment for specific workflows. 
...
| Automatic forwarding             | Allow users to automatically forward incoming email to another address                                                                            | Enabled                                                                                                                                                                                                                      | We will eventually disable this in favor of custom routing rules for domains where we want to allow forwarding. There is no mechanism for allow-listing destination domains, so we rely on alerting when new forwarding rules are added. |
