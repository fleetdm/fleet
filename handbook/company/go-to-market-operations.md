...
#### Event types and intent signals
...
> Historical event records are only created in the production environment. When deleting a contact's data (e.g. for a data deletion request), remember to also delete any related Historical event records associated with that contact.
