...
- **AI-generated code is not assembly.** Compilers are deterministic: they turn a fully descriptive higher-level specification into the same output every time. Generative AI does not. It reads the current state, decides on changes, and makes them, with no mutable spec that captures the exact behavior, edge cases, and intent of the system being changed. Reviewing only the prompt or a diff summary is not equivalent to reviewing the change.
- **Cognitive debt.** Anything generated by AI that you didn't write yourself starts as cognitive debt. You don't fully understand it yet. The question isn't whether to pay that debt, but when.
  - _As an author:_ after the AI generates the work, before you open the PR (or send the email, or publish the post). Read every line. If you can't explain why something is there, fix or remove it before asking someone else to review it.
...
- **AI review is not a substitute for human review.** AI reviews are useful, but they don't transfer accountability. There may come a day when AI review is reliable enough that line-by-line human review becomes unnecessary. We aren't there yet. "The AI reviewed it" is not a substitute for a human who understands what they're shipping.
