...
**Full draft** is reserved for new design patterns and complex changes. The PD assigns themselves, adds the `:product` label, and tracks the issue on the [drafting board](https://github.com/orgs/fleetdm/projects/67). When drafting is complete, the PD removes the `:product` label, takes the issue off the drafting board, and brings it to the next standup or planning meeting for user story review and [t-shirt sizing](#t-shirt-sizing-capacity-planning).

**Fast draft** is for stories with well-understood patterns, and for other new work that requires product design input (some bug fixes, internal tooling, etc.) The PD collaborates with the team (often asynchronously) on what guidance is needed to implement the change. This may be a Figma wireframe, a quick sketch, a bulleted list of changes, or a prototype built directly into the product to choose between options. The PD and EM are responsible for escalating to the HPD and/or CTO if needed. If a customer promise or activation blocker is fast-drafted, it should still be t-shirt sized to reduce risk of delayed delivery.
