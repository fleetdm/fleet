...
- If the solution is obvious when additional info is provided, write a message with only the failed **action** followed by it's **reason**. For example, "Couldn't run. Only users with the maintainer role and above can run scripts."
