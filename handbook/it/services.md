...
This page covers the IT services available at Fleet, including VPN access, certificate authorities, identity provider (IdP) capabilities and environments, Apple Business Manager, and other foundational technology services.
...
Fleet manages certificate authorities (CAs) for various purposes including device authentication, Wi-Fi connectivity, and secure communications. Certificate authority configurations are managed through Fleet's configuration management system.

Available certificate authority types include:
- **DigiCert**: Enterprise certificate management
- **NDES SCEP Proxy**: Microsoft Network Device Enrollment Service
- **Custom SCEP Proxy**: Custom Simple Certificate Enrollment Protocol implementations
- **Custom EST Proxy**: Custom Enrollment over Secure Transport implementations
...
Fleet uses identity provider services for authentication and access management. IdP configurations support various authentication methods including SAML SSO, conditional access, and certificate-based authentication.

IdP environments are configured to support:
- Single Sign-On (SSO) for internal tools and services
...
Fleet maintains an Apple Business Manager account for managing Apple device enrollment, app distribution, and device management capabilities. Access to Apple Business Manager is managed through 1Password.

Apple Business Manager is used for:
- Device enrollment program (DEP) management
- Volume Purchase Program (VPP) for app distribution
...
<meta name="title" value="Services">
