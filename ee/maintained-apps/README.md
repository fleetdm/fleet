...
| `cask_path`              | string        | Path (relative to the repo root) to a local file containing the cask JSON in the same schema as `https://formulae.brew.sh/api/cask/<token>.json`. Used to commit cask metadata for third-party taps directly into this repo under [`inputs/homebrew/custom-tap/`](inputs/homebrew/custom-tap/). Takes priority over `api_base_url`. See [Ingesting apps from a custom tap](#ingesting-apps-from-a-custom-tap) below. |

### Ingesting apps from a custom tap

Apps that live in a third-party Homebrew tap (not `Homebrew/homebrew-cask`) are not proxied by `https://formulae.brew.sh/api/`. There are two supported ways to ingest them:

**Option 1 (recommended): commit the cask into this repo.** Use this when you want the PR's validation CI to test the exact cask being merged, with no external HTTP dependency. Both the `.rb` source and the generated `.json` live in [`inputs/homebrew/custom-tap/`](inputs/homebrew/custom-tap/), laid out like a Homebrew tap:
...
See [`inputs/homebrew/custom-tap/README.md`](inputs/homebrew/custom-tap/README.md) for the full contributor flow.

**Option 2: fetch over HTTP from a tap-hosted endpoint.** Use this when the tap owner publishes a brew-API-compatible JSON endpoint and you want the ingester to pull the latest on every run.

1. Have the tap publish cask metadata at a stable URL. The JSON must match the shape returned by `https://formulae.brew.sh/api/cask/<token>.json`. A common pattern is to commit generated files under `api/cask/<token>.json` in the tap repo and serve them via `https://raw.githubusercontent.com/<owner>/<tap>/main/api/` or GitHub Pages.
2. In the app's input manifest, set `api_base_url` to the base URL (everything up to and including `/api/`, but without the `cask/<token>.json` suffix). The generator will fetch from that host for that app only.

In both cases, `go run cmd/maintained-apps/main.go --slug="<slug>"` produces the same output manifest as an app pulled from `formulae.brew.sh`. Apps without either field continue to use `formulae.brew.sh`.
