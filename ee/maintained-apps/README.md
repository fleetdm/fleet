...
- MSI installers: scripts are auto-generated by the ingester when possible (install and uninstall). You typically do not need to provide scripts.
- EXE installers: scripts are required. Fleet delivers the installer file to the host and sets `INSTALLER_PATH` in the environment. Your script must execute that file with the correct silent switches and return the installer’s exit code.

Example EXE install script `ee/maintained-apps/inputs/winget/scripts/example_install.ps1`:
```powershell
...
# The exact switches depend on the vendor (often "/S" or "/s"). For Inno Setup, a common set is:
$processOptions = @{
...
Example EXE uninstall script `ee/maintained-apps/inputs/winget/scripts/example_uninstall.ps1` (find and run the app’s uninstaller silently):
```powershell
# Attempts to locate the app's UninstallString from registry and execute it silently
param(
...
# Add common silent flags for uninstaller types
...
If you omit scripts for EXE installers, the generator will fail because it cannot infer the correct silent switches. Provide scripts for EXE; rely on auto-generated scripts for MSI where possible.
