...
> **Note on PR scope:** a checked box means Fleet now maintains the app — but **384** of the checked casks were added to `main` independently while this branch was open and were deduplicated out of this PR on the last `main` merge. They stay checked (the FMAs exist), but this PR itself contributes only the **376 net-new** ones. See [README.md](README.md#status-complete) for details.
