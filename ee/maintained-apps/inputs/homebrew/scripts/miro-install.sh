...
cleanup_backup_files() {
  ...
  if [ -d "$TMPDIR/Miro.app.bkp" ]; then
    echo "Removing existing backup file: $TMPDIR/Miro.app.bkp"
    sudo rm -rf "$TMPDIR/Miro.app.bkp" 2>/dev/null || true
  fi
  ...
  for search_base in /tmp /var/folders /private/var/folders; do
    if [ -d "$search_base" ]; then
      find "$search_base" -type d -name "Miro.app.bkp" -exec sudo rm -rf {} + 2>/dev/null || true
    fi
  done
}
...
quit_application 'com.electron.realtimeboard'
