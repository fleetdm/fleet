# NordPass installs per user, so its uninstall entry is in the installing user's
...
$displayNamePattern = '^NordPass$'
$publisher = "NordPass Team"
$taskName = "fleet-uninstall-nordpass"
...
try {
    ...
    if ($entries.Count -eq 0) {
        Write-Host "NordPass is not installed."
        Exit 0
    }
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) NordPass registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "NordPass removed."
    }
