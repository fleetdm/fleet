# Morgen installs per user, so its uninstall entry is in the installing user's
...
$displayNamePattern = '^Morgen \d'
$publisher = "Morgen AG"
$taskName = "fleet-uninstall-morgen"
...
try {
    ...
    if ($entries.Count -eq 0) {
        Write-Host "Morgen is not installed."
        Exit 0
    }
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) Morgen registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "Morgen removed."
    }
