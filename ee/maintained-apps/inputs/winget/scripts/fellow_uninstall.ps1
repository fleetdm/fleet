# Fellow installs per user, so its uninstall entry is in the installing user's
...
$displayNamePattern = '^Fellow$'
$publisher = "Fellow Insights Inc."
$taskName = "fleet-uninstall-fellow"
...
try {
    ...
    if ($entries.Count -eq 0) {
        Write-Host "Fellow is not installed."
        Exit 0
    }
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) Fellow registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "Fellow removed."
    }
