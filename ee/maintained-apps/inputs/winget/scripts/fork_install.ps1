...
#
# Fork installs per user and has no machine-wide mode, so installing it from this
...
try {
    ...
    if (-not $owner) {
        Throw "Fork installs per user and no user is signed in to this host. Sign in and try again."
    }
    $userAccount = "$($owner.Domain)\$($owner.User)"
    Write-Host "Installing Fork for $userAccount."
