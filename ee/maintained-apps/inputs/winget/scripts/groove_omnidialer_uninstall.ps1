# Groove OmniDialer installs per user, so its uninstall entry is in the installing user's
...
$displayNamePattern = '^Groove\ OmniDialer \d'
$publisher = "Groove Labs, Inc."
$taskName = "fleet-uninstall-groove-omnidialer"
...
try {
    ...
    if ($entries.Count -eq 0) {
        Write-Host "Groove OmniDialer is not installed."
        Exit 0
    }
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) Groove OmniDialer registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "Groove OmniDialer removed."
    }
