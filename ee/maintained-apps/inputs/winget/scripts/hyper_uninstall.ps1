# Locates Hyper's uninstaller from the registry and runs it silently.

# Match the registry DisplayName (osquery programs.name).
$softwareNameLike = "Hyper"
...
foreach ($key in $uninstallKeys) {
    if ($key.DisplayName -eq $softwareNameLike) {
        ...
        # Hyper is an electron-builder NSIS app. By default the NSIS uninstaller
        ...
        # gone, leaving the entry behind (the validator re-found 'Hyper' after
