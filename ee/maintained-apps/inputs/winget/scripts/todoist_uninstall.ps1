# Todoist installs per user, so its uninstall entry is in the installing user's
...
$displayNamePattern = '^Todoist \d'
$publisher = "Doist"
$taskName = "fleet-uninstall-todoist-app"
...
try {
    ...
    if ($entries.Count -eq 0) {
        Write-Host "Todoist is not installed."
        Exit 0
    }
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) Todoist registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "Todoist removed."
    }
