# Franz installs per user, so its uninstall entry is in the installing user's
...
$displayNamePattern = '^Franz \d'
$publisher = "Stefan Malzner"
$taskName = "fleet-uninstall-franz"
...
try {
    ...
    if ($entries.Count -eq 0) {
        Write-Host "Franz is not installed."
        Exit 0
    }
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) Franz registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "Franz removed."
    }
