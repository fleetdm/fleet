...
#
# Biscuit installs per user and has no machine-wide mode, so installing it from this
...
$taskName = "fleet-install-biscuit"
...
try {
    ...
    if (-not $owner) {
        Throw "Biscuit installs per user and no user is signed in to this host. Sign in and try again."
    }
    $userAccount = "$($owner.Domain)\$($owner.User)"
    Write-Host "Installing Biscuit for $userAccount."
