# Proton Mail installs per-user (winget exposes user scope only), so its
# uninstall entry lives under HKCU. Search HKCU first, then HKLM as a fallback.
...
foreach ($key in $uninstallKeys) {
    if ($key.DisplayName -like $softwareNameLike) {
        ...
        #   1. "C:\Users\...\Proton Mail\Uninstall Proton Mail.exe" /S  (quoted)
        #   2. C:\Users\...\Proton Mail\Uninstall.exe /S                (unquoted, may contain spaces)
