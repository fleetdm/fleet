# Miro installs per user, so its uninstall entry is in the installing user's
...
$displayNamePattern = '^Miro \d'
$publisher = "Miro"
$taskName = "fleet-uninstall-miro"
...
try {
    ...
    if ($entries.Count -eq 0) {
        Write-Host "Miro is not installed."
        Exit 0
    }
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) Miro registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "Miro removed."
    }
