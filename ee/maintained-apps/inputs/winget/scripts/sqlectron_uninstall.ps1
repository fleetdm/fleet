# Sqlectron installs per user, so its uninstall entry is in the installing user's
...
$displayNamePattern = '^sqlectron \d'
$publisher = "The Sqlectron Team"
$taskName = "fleet-uninstall-sqlectron"
...
try {
    ...
    if ($entries.Count -eq 0) {
        Write-Host "Sqlectron is not installed."
        Exit 0
    }
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) Sqlectron registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "Sqlectron removed."
    }
