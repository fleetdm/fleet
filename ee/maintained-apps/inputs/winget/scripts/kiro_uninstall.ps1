# Kiro installs per user, so its uninstall entry is in the installing user's
...
$displayNamePattern = '^Kiro\ \(User\)$'
$publisher = "Amazon Web Services"
$taskName = "fleet-uninstall-kiro"
...
try {
    ...
    if ($entries.Count -eq 0) {
        Write-Host "Kiro is not installed."
        Exit 0
    }
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) Kiro registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "Kiro removed."
    }
