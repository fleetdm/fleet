# Locates GoLand's NSIS uninstaller from the registry and runs it silently.
# JetBrains NSIS installers embed the version in DisplayName (e.g.
# "GoLand 2026.1.2"), so match by prefix and require the JetBrains publisher.

$softwareNameLike = "GoLand*"
...
Stop-Process -Name "goland64" -Force -ErrorAction SilentlyContinue
Stop-Process -Name "goland" -Force -ErrorAction SilentlyContinue
...
if ($uninstallCommand -match '^\s*"([^"]+)"\s*(.*)$') {
    ...
    # C:\Program Files\JetBrains\GoLand 2026.1.2\bin\Uninstall.exe
    $exePath = $matches[1]
    $existingArgs = $matches[2].Trim()
