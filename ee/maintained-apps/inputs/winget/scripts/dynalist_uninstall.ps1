# Dynalist installs per user, so its uninstall entry is in the installing user's
...
$displayNamePattern = '^Dynalist \d'
$publisher = "Dynalist Inc."
$taskName = "fleet-uninstall-dynalist"
...
try {
    ...
    if ($entries.Count -eq 0) {
        Write-Host "Dynalist is not installed."
        Exit 0
    }
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) Dynalist registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "Dynalist removed."
    }
