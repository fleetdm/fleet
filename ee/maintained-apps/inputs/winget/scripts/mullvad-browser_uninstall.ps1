# Mullvad Browser installs per user, so its uninstall entry is in the installing user's
...
$displayNamePattern = '^Mullvad\ Browser$'
$publisher = "Mullvad VPN"
$taskName = "fleet-uninstall-mullvad-browser"
...
try {
    ...
    if ($entries.Count -eq 0) {
        Write-Host "Mullvad Browser is not installed."
        Exit 0
    }
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) Mullvad Browser registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "Mullvad Browser removed."
    }
