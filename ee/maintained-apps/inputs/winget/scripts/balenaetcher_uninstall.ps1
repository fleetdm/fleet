# balenaEtcher installs per user, so its uninstall entry is in the installing user's
...
$displayNamePattern = '^balenaEtcher$'
$publisher = "Balena Ltd. <hello@balena.io>"
$taskName = "fleet-uninstall-balenaetcher"
...
try {
    ...
    if ($entries.Count -eq 0) {
        Write-Host "balenaEtcher is not installed."
        Exit 0
    }
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) balenaEtcher registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "balenaEtcher removed."
    }
