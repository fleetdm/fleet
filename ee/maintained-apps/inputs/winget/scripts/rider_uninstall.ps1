# Locates JetBrains Rider's NSIS uninstaller from the registry and runs it silently.
# JetBrains NSIS installers embed the version in DisplayName (e.g.
# "JetBrains Rider 2026.1.2"), so we match by prefix and require the JetBrains
# publisher to avoid collisions with other JetBrains IDEs.

$softwareNameLike = "JetBrains Rider*"
...
Stop-Process -Name "rider64" -Force -ErrorAction SilentlyContinue
Stop-Process -Name "rider" -Force -ErrorAction SilentlyContinue
...
if ($uninstallCommand -match '^\s*"([^"]+)"\s*(.*)$') {
    ...
    # C:\Program Files\JetBrains\Rider 2026.1.2\bin\Uninstall.exe
    $exePath = $matches[1]
    $existingArgs = $matches[2].Trim()
