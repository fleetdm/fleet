...
#
# Portfolio Performance installs per user and has no machine-wide mode, so installing it from this
...
$taskName = "fleet-install-portfolioperformance"
...
try {
    ...
    if (-not $owner) {
        Throw "Portfolio Performance installs per user and no user is signed in to this host. Sign in and try again."
    }
    $userAccount = "$($owner.Domain)\$($owner.User)"
    Write-Host "Installing Portfolio Performance for $userAccount."
