...
# msiexec to go idle. Only a Current Release entry counts, so an existing
# LTSR install doesn't register as this one finishing.
...
function Test-CitrixWorkspaceInstalled {
  ...
  foreach ($key in $uninstallKeys) {
    if ($key.DisplayName -eq $softwareName `
        ...
        -and $key.InstallSource -notlike '*\Ctx-*') {
      return $true
    }
  }
  return $false
