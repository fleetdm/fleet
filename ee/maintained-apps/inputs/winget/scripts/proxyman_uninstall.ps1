# Proxyman installs per user, so its uninstall entry is in the installing user's
...
$displayNamePattern = '^Proxyman \d'
$publisher = "Proxyman LLC"
$taskName = "fleet-uninstall-proxyman"
...
try {
    ...
    if ($entries.Count -eq 0) {
        Write-Host "Proxyman is not installed."
        Exit 0
    }
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) Proxyman registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "Proxyman removed."
    }
