# Insomnia installs per user, so its uninstall entry is in the installing user's
...
$displayNamePattern = '^Insomnia$'
$publisher = "Kong"
$taskName = "fleet-uninstall-insomnia"
...
try {
    ...
    if ($entries.Count -eq 0) {
        Write-Host "Insomnia is not installed."
        Exit 0
    }
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) Insomnia registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "Insomnia removed."
    }
