# Notion Calendar installs per user, so its uninstall entry is in the installing user's
...
$displayNamePattern = '^Notion\ Calendar \d'
$publisher = "Notion Labs, Inc."
$taskName = "fleet-uninstall-notion-calendar"
...
try {
    ...
    if ($entries.Count -eq 0) {
        Write-Host "Notion Calendar is not installed."
        Exit 0
    }
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) Notion Calendar registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "Notion Calendar removed."
    }
