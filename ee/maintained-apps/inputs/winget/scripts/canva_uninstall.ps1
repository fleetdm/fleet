# Canva installs per user, so its uninstall entry is in the installing user's
...
$displayNamePattern = '^Canva$'
$publisher = "Canva Pty Ltd"
$taskName = "fleet-uninstall-canva"
...
try {
    ...
    if ($entries.Count -eq 0) {
        Write-Host "Canva is not installed."
        Exit 0
    }
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) Canva registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "Canva removed."
    }
