# Locates RubyMine's NSIS uninstaller from the registry and runs it silently.
# JetBrains NSIS installers embed the version in DisplayName (e.g.
# "RubyMine 2026.1.2"), so we match by prefix and require the JetBrains
# publisher to avoid collisions with other JetBrains IDEs.

$softwareNameLike = "RubyMine*"
...
Stop-Process -Name "rubymine64" -Force -ErrorAction SilentlyContinue
Stop-Process -Name "rubymine" -Force -ErrorAction SilentlyContinue
...
if ($uninstallCommand -match '^\s*"([^"]+)"\s*(.*)$') {
    ...
    # C:\Program Files\JetBrains\RubyMine 2026.1.2\bin\Uninstall.exe
    $exePath = $matches[1]
    $existingArgs = $matches[2].Trim()
