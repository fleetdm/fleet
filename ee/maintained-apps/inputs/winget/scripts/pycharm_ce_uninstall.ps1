# Locates PyCharm Community Edition's NSIS uninstaller from the registry and runs it silently.
# JetBrains NSIS installers embed the version in DisplayName (e.g.
# "PyCharm Community Edition 2026.1.2"), so we match by prefix and require the JetBrains
# publisher to avoid collisions with other JetBrains IDEs.

$softwareNameLike = "PyCharm Community Edition*"
...
if ($uninstallCommand -match '^\s*"([^"]+)"\s*(.*)$') {
    ...
    # C:\Program Files\JetBrains\PyCharm Community Edition 2026.1.2\bin\Uninstall.exe
    $exePath = $matches[1]
    $existingArgs = $matches[2].Trim()
