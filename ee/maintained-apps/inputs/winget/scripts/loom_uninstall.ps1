# Loom installs per user, so its uninstall entry is in the installing user's
...
$displayNamePattern = '^Loom \d'
$publisher = "Loom, Inc."
$taskName = "fleet-uninstall-loom"
...
try {
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) Loom registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "Loom removed."
    }
