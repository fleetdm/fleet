# Portfolio Performance installs per user, so its uninstall entry is in the installing user's
...
#
# This installer refuses to run without elevation, so the task runs the
# signed-in user with an elevated token rather than their plain one.

$displayNamePattern = '^Portfolio\ Performance$'
$publisher = "Andreas Buchen"
$taskName = "fleet-uninstall-portfolioperformance"
...
try {
    ...
    if ($entries.Count -eq 0) {
        Write-Host "Portfolio Performance is not installed."
        Exit 0
    }
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) Portfolio Performance registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "Portfolio Performance removed."
    }
