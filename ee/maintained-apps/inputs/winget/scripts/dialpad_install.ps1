...
#
# Dialpad installs per user and has no machine-wide mode, so installing it from this
...
$taskName = "fleet-install-dialpad"
...
try {
    ...
    if (-not $owner) {
        Throw "Dialpad installs per user and no user is signed in to this host. Sign in and try again."
    }
    $userAccount = "$($owner.Domain)\$($owner.User)"
    Write-Host "Installing Dialpad for $userAccount."
