# Telegram installs per user, so its uninstall entry is in the installing user's
...
$displayNamePattern = '^Telegram\ Desktop$'
$publisher = "Telegram FZ-LLC"
$taskName = "fleet-uninstall-telegram"
...
function Resolve-Uninstaller {
    ...
    if ($arguments -notmatch '(?i)/VERYSILENT') {
...
try {
    ...
    if ($entries.Count -eq 0) {
        Write-Host "Telegram is not installed."
        Exit 0
    }
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) Telegram registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "Telegram removed."
    }
