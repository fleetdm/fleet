# Discord installs per user, so its uninstall entry is in the installing user's
...
$taskName = "fleet-uninstall-discord"
...
try {
    ...
    if ($remaining.Count -gt 0) {
        Write-Host "WARNING: $($remaining.Count) Discord registration(s) still present:"
        $remaining | ForEach-Object { Write-Host "  - $($_.DisplayName) [$($_.KeyPath)]" }
        if ($exitCode -eq 0) { $exitCode = 1 }
    } else {
        Write-Host "Discord removed."
    }
