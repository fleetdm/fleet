...
- **5.3.1 / 5.3.2 storage encryption** — CIS removed the old CoreStorage recommendation and split disk encryption into internal (5.3.1) and external (5.3.2). Fleet's 5.3.1 covers internal APFS volumes (see caveat below); the old CoreStorage policy was removed. Apple's sealed, read-only asset cryptex volumes (no role, names ending in `Cryptex`; present on macOS 15 and later) cannot be encrypted and are excluded so the policy does not false-fail. 5.3.2 (external) and 5.3.3 (FAT32/ExFAT) are documented under Limitations.
