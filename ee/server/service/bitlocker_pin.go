...
func (svc *Service) BitLockerPINStateForDevice(
	...
	// The cleanups cron retires abandoned submissions, but the page should not have to wait for it. A pending request past its
	// TTL is already uncollectable, and a delivered one past the result timeout is no longer waited on, so report either as
	// timed out rather than leaving the modal spinning.
