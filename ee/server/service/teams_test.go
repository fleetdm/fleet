...
// TestApplyTeamSpecsDoesNotStoreSoftwareInConfig verifies that applying a team
// spec with software does not persist that software into the team config. The
// source of truth for a team's software lives in the software tables (applied
// via the dedicated batch endpoints); storing it in the config produces stale
// data when read back. Regression test for
// https://github.com/fleetdm/fleet/issues/44970.
func TestApplyTeamSpecsDoesNotStoreSoftwareInConfig(t *testing.T) {
	authorizer, err := authz.NewAuthorizer()
	require.NoError(t, err)
	adminUser := &fleet.User{ID: 1, GlobalRole: new(fleet.RoleAdmin)}
	ctx := test.UserContext(context.Background(), adminUser)
	...
	softwareSpec := &fleet.SoftwareSpec{
		AppStoreApps: optjson.SetSlice([]fleet.TeamSpecAppStoreApp{
			{AppStoreID: "123", Platform: "darwin", InstallDuringSetup: optjson.SetBool(true)},
		}),
	}

	t.Run("new team does not persist software in config", func(t *testing.T) {
		svc, ds := newSvc()
		ds.TeamByNameFunc = func(ctx context.Context, name string) (*fleet.Team, error) {
			return nil, &notFoundError{}
		}
		var newTeam *fleet.Team
		ds.NewTeamFunc = func(ctx context.Context, team *fleet.Team) (*fleet.Team, error) {
			newTeam = team
			team.ID = 1
			return team, nil
		...
		_, err := svc.ApplyTeamSpecs(ctx, []*fleet.TeamSpec{
			{Name: "team1", Software: softwareSpec},
		}, fleet.ApplyTeamSpecOptions{})
		require.NoError(t, err)
		require.True(t, ds.NewTeamFuncInvoked)
		require.NotNil(t, newTeam)
		require.Nil(t, newTeam.Config.Software, "software must not be stored in the team config")
	})

	t.Run("existing team does not persist software in config", func(t *testing.T) {
		svc, ds := newSvc()
		existing := &fleet.Team{ID: 7, Name: "team1"}
		ds.TeamByNameFunc = func(ctx context.Context, name string) (*fleet.Team, error) {
			return existing, nil
		}
		var savedTeam *fleet.Team
		...
		require.True(t, ds.SaveTeamFuncInvoked)
		require.NotNil(t, savedTeam)
		require.Nil(t, savedTeam.Config.Software, "software must not be stored in the team config")
	})
}
