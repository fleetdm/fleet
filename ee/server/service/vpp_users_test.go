...
func TestEnsureVPPClientUser_NewUser(t *testing.T) {
	...
	setupFakeVPPServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		require.NotEmpty(t, got.Users[0].ClientUserId)
		require.Equal(t, managedAppleID, got.Users[0].ManagedAppleId)
...
func TestEnsureVPPClientUser_PendingRetryReusesUUID(t *testing.T) {
	...
	setupFakeVPPServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		require.NoError(t, json.NewDecoder(r.Body).Decode(&got))
		require.Equal(t, priorUUID, got.Users[0].ClientUserId, "retry must reuse the prior clientUserId")
