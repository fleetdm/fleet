...
func (d *Datastore) EnrollHost(ctx context.Context, osqueryHostID, hardwareUUID, hardwareSerial, nodeKey string, teamID *uint, cooldown time.Duration) (*fleet.Host, error) {
	h, err := d.Datastore.EnrollHost(ctx, osqueryHostID, hardwareUUID, hardwareSerial, nodeKey, teamID, cooldown)
	if err == nil && d.enforceHostLimit > 0 {
		if err := addHosts(ctx, d.pool, h.ID); err != nil {
			logging.WithErr(ctx, err)
		}
