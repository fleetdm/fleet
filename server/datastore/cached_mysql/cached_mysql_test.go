...
func TestCachedTeamLite(t *testing.T) {
	t.Parallel()

	mockedDS := new(mock.Store)
	ds := New(mockedDS, WithTeamLiteExpiration(100*time.Millisecond))

	testTeamLite := fleet.TeamLite{
		ID:   1,
		Name: "team1",
	...
	deleted := false
	mockedDS.TeamLiteFunc = func(ctx context.Context, teamID uint) (*fleet.TeamLite, error) {
		if deleted {
			return nil, errors.New("not found")
		}
		return &testTeamLite, nil
	}
	mockedDS.SaveTeamFunc = func(ctx context.Context, team *fleet.Team) (*fleet.Team, error) {
		return team, nil
	}
	...
	// get the team, will load it into cache
	team, err := ds.TeamLite(t.Context(), 1)
	require.NoError(t, err)
	require.Equal(t, testTeamLite, *team)
	require.True(t, mockedDS.TeamLiteFuncInvoked)
	mockedDS.TeamLiteFuncInvoked = false

	// get it again, will get it from cache (as a clone)
	team, err = ds.TeamLite(t.Context(), 1)
	require.NoError(t, err)
	require.Equal(t, testTeamLite, *team)
	require.NotSame(t, &testTeamLite, team)
	require.False(t, mockedDS.TeamLiteFuncInvoked)

	// changing a deep value on the returned clone doesn't affect the cached value
	team.Config.WebhookSettings.HostActivitiesWebhook.DestinationURL = "https://example.com/changed"
	team, err = ds.TeamLite(t.Context(), 1)
	require.NoError(t, err)
	require.Equal(t, "https://example.com/hook", team.Config.WebhookSettings.HostActivitiesWebhook.DestinationURL)
	require.False(t, mockedDS.TeamLiteFuncInvoked)

	// a different team ID misses the cache
	_, err = ds.TeamLite(t.Context(), 2)
	require.NoError(t, err)
	require.True(t, mockedDS.TeamLiteFuncInvoked)
	mockedDS.TeamLiteFuncInvoked = false

	// saving the team invalidates its entry
	_, err = ds.SaveTeam(t.Context(), &fleet.Team{ID: 1})
	require.NoError(t, err)
	_, err = ds.TeamLite(t.Context(), 1)
	require.NoError(t, err)
	require.True(t, mockedDS.TeamLiteFuncInvoked)
	mockedDS.TeamLiteFuncInvoked = false

	// deleting the team invalidates its entry; the next read hits the DB and errors
	err = ds.DeleteTeam(t.Context(), 1)
	require.NoError(t, err)
	_, err = ds.TeamLite(t.Context(), 1)
	require.Error(t, err)
	require.True(t, mockedDS.TeamLiteFuncInvoked)
	...
	// expiration: reload, wait past TTL, next read hits the DB again
	deleted = false
	_, err = ds.TeamLite(t.Context(), 1)
	require.NoError(t, err)
	mockedDS.TeamLiteFuncInvoked = false
