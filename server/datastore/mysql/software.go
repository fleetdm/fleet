	...
	if opts.OnlyAvailableForInstall || opts.IncludeAvailableForInstall {
		...
		if !opts.VulnerableOnly {
			stmtAvailable = `
				...
				EXISTS(SELECT 1 FROM script_contents sc WHERE sc.id = si.uninstall_script_content_id AND sc.contents != '') as package_has_uninstall_script,
				vat.self_service as vpp_app_self_service,
		...
		if len(softwareTitleIDs) > 0 {
			replacements = append(replacements,
				...
				SELECT
					...
					EXISTS(SELECT 1 FROM script_contents sc WHERE sc.id = software_installers.uninstall_script_content_id AND sc.contents != '') AS package_has_uninstall_script,
