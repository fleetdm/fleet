		...
		return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
			...
			if len(newTitlesNeeded) > 0 {
				...
				for tk := range uniqueTitlesToInsert {
					...
					queryArgs = append(queryArgs, firstArg, title.Source, title.Browser, bundleID)
				}

				queryTitles := fmt.Sprintf(`SELECT id, name, source, browser, bundle_identifier
					FROM software_titles
					WHERE (COALESCE(bundle_identifier, name), source, browser, COALESCE(bundle_identifier, '')) IN (%s)`, titlePlaceholders)
				...
				for _, td := range titlesData {
					...
					for checksum, title := range newTitlesNeeded {
						...
						if nameMatches && td.Source == title.Source && td.Browser == title.Browser && bundleID == titleBundleID {
