...
`
			...
			for _, hostIdentifiers := range batchHostnames(s.Hosts) {
				...
				sql = `
INSERT IGNORE INTO label_membership (label_id, host_id) (SELECT DISTINCT ?, id FROM hosts where hostname IN (?) OR hardware_serial IN (?) OR uuid IN (?) OR CAST(id AS CHAR) IN (?))`
