...
func testTargetsCountHosts(t *testing.T, ds *Datastore) {
	...
	assert.Equal(t, uint(3), metrics.OfflineHosts) // offline hosts also includes mia hosts as of Fleet 4.15
	...
	require.Nil(t, err)
	assert.Equal(t, uint(6), metrics.TotalHosts)
	assert.Equal(t, uint(3), metrics.OfflineHosts)
	assert.Equal(t, uint(3), metrics.OnlineHosts)
	assert.Equal(t, uint(1), metrics.MissingInActionHosts)
	...
	assert.Equal(t, uint(6), metrics.TotalHosts)
	assert.Equal(t, uint(0), metrics.OnlineHosts)
	assert.Equal(t, uint(6), metrics.OfflineHosts) // offline hosts also includes mia hosts as of Fleet 4.15
	assert.Equal(t, uint(1), metrics.MissingInActionHosts)
...
func testTargetsHostStatus(t *testing.T, ds *Datastore) {
	...
	expectMIA := fleet.TargetMetrics{TotalHosts: 1, OfflineHosts: 1, MissingInActionHosts: 1} // offline hosts also includes mia hosts as of Fleet 4.15
