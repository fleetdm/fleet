...
func TestQueries(t *testing.T) {
	...
	cases := []struct {
		...
		{"HasAnyLabelScopedScheduledQueries", testHasAnyLabelScopedScheduledQueries},
...
func testHasAnyLabelScopedScheduledQueries(t *testing.T, ds *Datastore) {
	ctx := context.Background()
	user := test.NewUser(t, ds, "Gabriel", "gabriel@fleet.co", true)

	// empty: no label-scoped scheduled queries
	got, err := ds.HasAnyLabelScopedScheduledQueries(ctx)
	require.NoError(t, err)
	require.False(t, got)
