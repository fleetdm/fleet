...
// ReplicaStatus is a struct that holds the status information of the read replica, retrieved by SHOW SLAVE STATUS
type ReplicaStatus struct {
	ExecSourceLogPosition uint64
}
...
func (ds *Datastore) ReplicaSync(ctx context.Context) error {
	if ds.primary == ds.replica {
		// No replica present, nothing to sync
		return nil
	}
	// Grab the current primary's binlog position
	...
	if rs.ExecSourceLogPosition < ms.Position {
		...
		go func() {
			for rs.ExecSourceLogPosition < ms.Position {
...
func (ds *Datastore) ReplicaStatus(ctx context.Context) (ReplicaStatus, error) {
	replica := ds.replica.(*sqlx.DB)

	// The 'SHOW SLAVE STATUS' command has been deprecated and replaced by 'SHOW REPLICA STATUS' in MySQL 8.0.22
	rows, err := replica.Query("SHOW SLAVE STATUS")
	if err != nil {
		return ReplicaStatus{}, ctxerr.Wrap(ctx, err, "show replica status")
	}
	defer rows.Close()
	...
	columns, err := rows.Columns()
	if err != nil {
		return status, ctxerr.Wrap(ctx, err, "get columns")
	}
	numberOfColumns := len(columns)
	for rows.Next() {
		cols := make([]interface{}, numberOfColumns)
		...
		for i, col := range cols {
			...
			case "Exec_Master_Log_Pos", // MySQL 5.7 name
				...
				status.ExecSourceLogPosition, err = strconv.ParseUint(pos.String, 10, 64)
				if err != nil {
	...
	if err := rows.Err(); err != nil {
		return status, ctxerr.Wrap(ctx, err, "rows error")
	}
	if status.ExecSourceLogPosition == 0 {
		return status, ctxerr.New(ctx, "missing required fields in replica status")
	}
	return status, nil
}
