...
VALUES
  (?, ?, ?, ?, ?)`, host.ID, incomingKey.Base, clientError, decryptable, incomingKey.CreatedAt)
		...
		case errors.As(err, &mysqlErr) && mysqlErr.Number == 1062:
			ds.slogger().ErrorContext(ctx, "Primary key already exists in host_disk_encryption_keys. Falling back to update", "host_id", host)
...
VALUES
  (?, ?, ?, ?, TRUE, ?)`, host.ID, incomingKey.Base, incomingKey.Salt, incomingKey.KeySlot, incomingKey.CreatedAt)
		...
		switch {
		case errors.As(err, &mysqlErr) && mysqlErr.Number == 1062:
			ds.slogger().ErrorContext(ctx, "Primary key already exists in LUKS host_disk_encryption_keys. Falling back to update", "host_id", host)
			// This should never happen unless there is a bug in the code or an infra issue (like huge replication lag).
		default:
...
LIMIT 1`
	...
	if err == sql.ErrNoRows && host.HardwareSerial != "" {
		...
		ds.slogger().DebugContext(ctx, "get archived disk encryption key by host serial", "serial", host.HardwareSerial, "host_id", host.ID)
