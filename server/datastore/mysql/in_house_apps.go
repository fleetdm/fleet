...
func (ds *Datastore) markAllPendingInHouseAppInstallsAsFailedForHost(ctx context.Context, tx sqlx.ExtContext,
...
`
	...
	if err := sqlx.SelectContext(ctx, tx, &failedCmds, loadFailedCmdsStmt, hostID); err != nil {
		return nil, nil, ctxerr.Wrap(ctx, err, "load pending in house install commands for host")
	}
...
`
	if _, err := tx.ExecContext(ctx, installFailStmt, hostID); err != nil {
		return nil, nil, ctxerr.Wrap(ctx, err, "set all in house install as failed")
	}
	...
	for _, cmd := range failedCmds {
		...
		var (
			user *fleet.User
			act  *fleet.ActivityDetails
			err  error
		)
		user, act, err = ds.getPastActivityDataForInHouseAppInstallDB(ctx, tx, &mdm.CommandResults{CommandUUID: cmd, Status: fleet.MDMAppleStatusError}, softwareTypeInHouseApp)
		if err != nil {
...
func (ds *Datastore) getPastActivityDataForInHouseAppInstallDB(ctx context.Context, q sqlx.QueryerContext, commandResults *mdm.CommandResults, softwareType softwareType,
) (*fleet.User, *fleet.ActivityDetails, error) {
	if commandResults == nil {
		return nil, nil, nil
	}
...
`
	// TODO: rename vpa, use hsi instead of hvsi
	inHouseJoin := `
	LEFT OUTER JOIN in_house_apps vpa ON hvsi.in_house_app_id = vpa.id AND vpa.team_id = h.team_id
`
...
`
	...
	case softwareTypeInHouseApp:
		fmt.Sprintf(baseStmt, inHouseSelect, "host_in_house", inHouseJoin)
	case softwareTypeVPP:
		fmt.Sprintf(baseStmt, vppSelect, "host_vpp", vppJoin)
	default:
	...
	listStmt, args, err := sqlx.Named(baseStmt, map[string]any{
		"command_uuid":              commandResults.CommandUUID,
		"software_status_failed":    string(fleet.SoftwareInstallFailed),
		"software_status_installed": string(fleet.SoftwareInstalled),
	})
	...
	switch softwareType {
	case softwareTypeInHouseApp:
		fmt.Sprintf(baseStmt, inHouseSelect, inHouseJoin)
	...
	case softwareTypeVPP:
		fmt.Sprintf(baseStmt, vppSelect, vppJoin)
