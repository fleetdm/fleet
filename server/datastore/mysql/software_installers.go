...
WHERE
  si.title_id = ? AND si.global_or_team_id = ?`,
	...
	if len(inclAny) > 0 && len(exclAny) > 0 {
		...
		ds.slogger().WarnContext(ctx, "software installer has both include and exclude labels", "installer_id", dest.InstallerID, "include", fmt.Sprintf("%v", inclAny), "exclude", fmt.Sprintf("%v", exclAny))
	}
