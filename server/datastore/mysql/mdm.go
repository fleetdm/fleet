	...
	if len(profileUUIDs) > 0 {
		...
		for _, puid := range profileUUIDs {
			if strings.HasPrefix(puid, fleet.MDMAppleProfileUUIDPrefix) { //nolint:gocritic // ignore ifElseChain
				...
				hasAppleDecls = true
				declProfUUIDs = append(declProfUUIDs, puid)
...
OR
	...
	case hasAppleDecls:
		// Fetch hosts affected by the declaration changes so we can scope the
		// declaration state query to only those hosts instead of scanning all hosts.
		uuidStmt = `
SELECT DISTINCT h.uuid, h.platform
FROM hosts h
...
WHERE
	...
	AND (h.platform = 'darwin' OR h.platform = 'ios' OR h.platform = 'ipados')`
		args = append(args, declProfUUIDs)
	}
