...
func testStatisticsShouldSend(t *testing.T, ds *Datastore) {
	...
	assert.Equal(t, 1, stats.NumSoftwareVersions)
	assert.Equal(t, 1, stats.NumHostSoftwares)
	assert.Equal(t, 1, stats.NumSoftwareTitles)
	assert.Equal(t, 1, stats.NumHostSoftwareInstalledPaths)
	assert.Equal(t, 1, stats.NumSoftwareCPEs)
	assert.Equal(t, 1, stats.NumSoftwareCVEs)
