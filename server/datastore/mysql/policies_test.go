...
func testPoliciesMembershipView(deferred bool, t *testing.T, ds *Datastore) {
	...
	require.NoError(t, ds.RecordPolicyQueryExecutions(ctx, host1, map[uint]*bool{p.ID: ptr.Bool(true)}, time.Now(), deferred, nil))
	require.NoError(t, ds.RecordPolicyQueryExecutions(ctx, host1, map[uint]*bool{p.ID: ptr.Bool(true)}, time.Now(), deferred, nil))

	require.NoError(t, ds.RecordPolicyQueryExecutions(ctx, host2, map[uint]*bool{p.ID: nil}, time.Now(), deferred, nil))
	require.NoError(t, ds.RecordPolicyQueryExecutions(ctx, host2, map[uint]*bool{p.ID: ptr.Bool(false)}, time.Now(), deferred, nil))
	require.NoError(t, ds.RecordPolicyQueryExecutions(ctx, host2, map[uint]*bool{p.ID: ptr.Bool(true)}, time.Now(), deferred, nil))
	...
	require.NoError(t, ds.RecordPolicyQueryExecutions(ctx, host1, map[uint]*bool{p.ID: ptr.Bool(false)}, time.Now(), deferred, nil))
	require.NoError(t, ds.RecordPolicyQueryExecutions(ctx, host2, map[uint]*bool{p2.ID: ptr.Bool(false)}, time.Now(), deferred, nil))
	...
	require.NoError(t, ds.RecordPolicyQueryExecutions(ctx, host3, map[uint]*bool{t1pol.ID: ptr.Bool(true), p.ID: ptr.Bool(true), p2.ID: ptr.Bool(false)}, time.Now(), deferred, nil))
	require.NoError(t, ds.RecordPolicyQueryExecutions(ctx, host4, map[uint]*bool{t2pol.ID: ptr.Bool(false), t2pol2.ID: ptr.Bool(true), p.ID: ptr.Bool(false)}, time.Now(), deferred, nil))
	require.NoError(t, ds.RecordPolicyQueryExecutions(ctx, host5, map[uint]*bool{t2pol.ID: ptr.Bool(true), t2pol2.ID: ptr.Bool(true), p2.ID: ptr.Bool(true)}, time.Now(), deferred, nil))
...
func testListMergedTeamPolicies(t *testing.T, ds *Datastore) {
	...
	err = ds.RecordPolicyQueryExecutions(ctx, host, map[uint]*bool{gpol.ID: ptr.Bool(true)}, time.Now(), false, nil)
	require.NoError(t, err)
	...
	err = ds.RecordPolicyQueryExecutions(ctx, host, map[uint]*bool{team1policy.ID: ptr.Bool(true)}, time.Now(), false, nil)
	require.NoError(t, err)
...
func testPolicyQueriesForHost(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, ds.RecordPolicyQueryExecutions(context.Background(), host1, map[uint]*bool{tp.ID: ptr.Bool(false), gp.ID: nil}, time.Now(), false, nil))
	...
	require.NoError(t, ds.RecordPolicyQueryExecutions(context.Background(), host2, map[uint]*bool{gp.ID: ptr.Bool(true)}, time.Now(), false, nil))
...
func testPoliciesByID(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, ds.RecordPolicyQueryExecutions(context.Background(), host1, map[uint]*bool{policy1.ID: ptr.Bool(true)}, time.Now(), false, nil))
...
func testTeamPolicyTransfer(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, ds.RecordPolicyQueryExecutions(ctx, host1, map[uint]*bool{team1Policy.ID: ptr.Bool(false), globalPolicy.ID: ptr.Bool(true)}, time.Now(), false, nil))
	require.NoError(t, ds.RecordPolicyQueryExecutions(ctx, host1, map[uint]*bool{team1Policy.ID: ptr.Bool(true), globalPolicy.ID: ptr.Bool(true)}, time.Now(), false, nil))
	require.NoError(t, ds.RecordPolicyQueryExecutions(ctx, host2, map[uint]*bool{team1Policy.ID: ptr.Bool(false), globalPolicy.ID: ptr.Bool(true)}, time.Now(), false, nil))
	require.NoError(t, ds.RecordPolicyQueryExecutions(ctx, host2, map[uint]*bool{team1Policy.ID: ptr.Bool(true), globalPolicy.ID: ptr.Bool(true)}, time.Now(), false, nil))
...
func testCachedPolicyCountDeletesOnPolicyChange(t *testing.T, ds *Datastore) {
	...
	require.NoError(
		t, ds.RecordPolicyQueryExecutions(
			ctx, teamHost, map[uint]*bool{globalPolicy.ID: ptr.Bool(false), globalPolicy.ID: ptr.Bool(false)}, time.Now(), false, nil,
		),
	)
	require.NoError(
		t, ds.RecordPolicyQueryExecutions(
			ctx, teamHost, map[uint]*bool{teamPolicy.ID: ptr.Bool(false), teamPolicy.ID: ptr.Bool(false)}, time.Now(), false, nil,
		),
	)
	require.NoError(
		t, ds.RecordPolicyQueryExecutions(
			ctx, globalHost, map[uint]*bool{globalPolicy.ID: ptr.Bool(false), globalPolicy.ID: ptr.Bool(false)}, time.Now(), false, nil,
		),
	)
...
func testPolicyViolationDays(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, ds.RecordPolicyQueryExecutions(context.Background(), hosts[0], map[uint]*bool{pol.ID: ptr.Bool(false)}, then, false, nil))
	require.NoError(t, ds.RecordPolicyQueryExecutions(context.Background(), hosts[1], map[uint]*bool{pol.ID: ptr.Bool(true)}, then, false, nil))
	require.NoError(t, ds.RecordPolicyQueryExecutions(context.Background(), hosts[2], map[uint]*bool{pol.ID: ptr.Bool(true)}, then, false, nil))
	...
	require.NoError(t, ds.RecordPolicyQueryExecutions(ctx, hosts[1], map[uint]*bool{pol.ID: ptr.Bool(false)}, time.Now(), false, nil))
	require.NoError(t, setStatsTimestampDB(time.Now().Add(-25*time.Hour)))
	require.NoError(t, ds.IncrementPolicyViolationDays(ctx))
	actual, possible, err = amountPolicyViolationDaysDB(ctx, ds.reader(ctx))
	require.NoError(t, err)
	...
	require.NoError(t, ds.RecordPolicyQueryExecutions(ctx, hosts[1], map[uint]*bool{pol.ID: ptr.Bool(true)}, time.Now(), false, nil))
	require.NoError(t, setStatsTimestampDB(time.Now().Add(-25*time.Hour)))
	require.NoError(t, ds.IncrementPolicyViolationDays(ctx))
	actual, possible, err = amountPolicyViolationDaysDB(ctx, ds.reader(ctx))
	require.NoError(t, err)
...
func testOutdatedAutomationBatch(t *testing.T, ds *Datastore) {
	...
	err = ds.RecordPolicyQueryExecutions(ctx, h1, map[uint]*bool{pol1.ID: ptr.Bool(false), pol2.ID: ptr.Bool(true)}, time.Now(), false, nil)
	require.NoError(t, err)
	err = ds.RecordPolicyQueryExecutions(ctx, h2, map[uint]*bool{pol1.ID: ptr.Bool(false), pol2.ID: ptr.Bool(false)}, time.Now(), false, nil)
	require.NoError(t, err)
...
func testGetTeamHostsPolicyMembershipsEmailPriority(t *testing.T, ds *Datastore) {
	...
	newHost := func(name string) *fleet.Host {
		...
		err = ds.RecordPolicyQueryExecutions(ctx, h, map[uint]*bool{calendarPolicy.ID: ptr.Bool(false)}, time.Now(), false, nil)
		require.NoError(t, err)
		return h
	}
...
func testPolicyLabelMembershipCleanup(t *testing.T, ds *Datastore) {
	...
	for _, h := range []*fleet.Host{hostNoLabels, hostLabel1, hostLabel2, hostLabelBoth} {
		err = ds.RecordPolicyQueryExecutions(ctx, h, map[uint]*bool{policy.ID: ptr.Bool(true)}, time.Now(), false, nil)
		require.NoError(t, err)
	}
	...
	for _, h := range []*fleet.Host{hostLabel1, hostLabelBoth, hostWinLabel1, hostMacLabel1} {
		err = ds.RecordPolicyQueryExecutions(ctx, h, map[uint]*bool{policy2.ID: ptr.Bool(true)}, time.Now(), false, nil)
		require.NoError(t, err)
	}
...
func testDeletePolicyWithSoftwareActivatesNextActivity(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, ds.RecordPolicyQueryExecutions(ctx, hostNoTm, map[uint]*bool{policyNoTm.ID: ptr.Bool(false)}, time.Now(), false, nil))
	require.NoError(t, ds.RecordPolicyQueryExecutions(ctx, hostTm, map[uint]*bool{policyTm.ID: ptr.Bool(false)}, time.Now(), false, nil))
...
func testIsPolicyFailing(t *testing.T, ds *Datastore) {
	...
	// Not failing
	err = ds.RecordPolicyQueryExecutions(ctx, host, map[uint]*bool{policy.ID: ptr.Bool(true)}, time.Now(), false, nil)
	require.NoError(t, err)
...
func testResetAttemptsOnFailingToPassingSync(t *testing.T, ds *Datastore) {
	...
	// p1 will be failing
	require.NoError(t, ds.RecordPolicyQueryExecutions(ctx, host, map[uint]*bool{p1.ID: ptr.Bool(false)}, time.Now(), false, nil))
	...
	// p1 is now passing
	err = ds.RecordPolicyQueryExecutions(ctx, host, map[uint]*bool{p1.ID: ptr.Bool(true), p2.ID: ptr.Bool(true)}, time.Now(), false, nil)
	require.NoError(t, err)
...
func testBatchedPolicyMembershipCleanup(t *testing.T, ds *Datastore) {
	...
	for _, h := range hosts {
		err := ds.RecordPolicyQueryExecutions(ctx, h, map[uint]*bool{pol.ID: ptr.Bool(false)}, time.Now(), false, nil)
		require.NoError(t, err)
	}
...
func testBatchedPolicyMembershipCleanupOnPolicyUpdate(t *testing.T, ds *Datastore) {
	...
	for _, h := range labelHosts {
		err := ds.RecordPolicyQueryExecutions(ctx, h, map[uint]*bool{lblPol.ID: ptr.Bool(false)}, time.Now(), false, nil)
		require.NoError(t, err)
	}
...
func testCleanupPolicyMembershipCrashRecovery(t *testing.T, ds *Datastore) {
	...
	recordResults := func(t *testing.T, hosts []*fleet.Host, polID uint) {
		t.Helper()
		for _, h := range hosts {
			err := ds.RecordPolicyQueryExecutions(ctx, h, map[uint]*bool{polID: ptr.Bool(false)}, time.Now(), false, nil)
			require.NoError(t, err)
		}
	}
