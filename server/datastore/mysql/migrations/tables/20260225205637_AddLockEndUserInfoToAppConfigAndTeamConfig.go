...
func init() {
	MigrationClient.AddMigration(Up_20260225205637, Down_20260225205637)
}

func Up_20260225205637(tx *sql.Tx) error {
	...
	err := updateAppConfigJSON(tx, func(config *fleet.AppConfig) error {
		if config != nil {
			config.MDM.MacOSSetup.LockEndUserInfo = optjson.SetBool(config.MDM.MacOSSetup.EnableEndUserAuthentication)
		}
		return nil
	})
	if err != nil {
	...
	// Update team config using same logic
...
func Down_20260225205637(tx *sql.Tx) error {
	return nil
}
