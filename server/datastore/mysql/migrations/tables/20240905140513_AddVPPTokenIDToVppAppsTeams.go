...
ALTER TABLE vpp_apps_teams
	...
	if _, err := tx.Exec(stmtAddColumn); err != nil {
		return fmt.Errorf("failed to add vpp_token_id column to table")
	}
	...
	if _, err := tx.Exec(stmtAssociate); err != nil {
		return fmt.Errorf("failed to associate vpp apps with first token")
	}

	if _, err := tx.Exec(stmtAddConstraint); err != nil {
		return fmt.Errorf("failed to add vpp token id constraint")
	}
