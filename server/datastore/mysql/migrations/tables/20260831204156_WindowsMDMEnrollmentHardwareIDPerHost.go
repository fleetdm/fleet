...
func Up_20260831204156(tx *sql.Tx) error {
	// The old UNIQUE KEY on mdm_hardware_id alone assumed one HWDevID identifies one device forever. The HWDevID is cached in the OS
	// enrollment state, so machines cloned from an image that was already enrolled in MDM all present the image's value, and under the old
	// key the second one to enroll took over the first one's enrollment row and silently stopped Fleet from managing it. Keying on
	// (mdm_hardware_id, host_uuid) lets both hosts hold their own enrollment. Existing rows need no dedup pass: uniqueness on
