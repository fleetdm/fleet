...
CREATE TABLE windows_mdm_commands (
    ...
    -- soon as it is enqueued. It is unique per device, unlike macOS commands
    -- that may target multiple hosts. Note that a UUID is 36 chars, but for
    -- consistency with nano_mdm_commands we use 127 here.
    command_uuid    VARCHAR(127) NOT NULL,
    device_id       VARCHAR(255) NOT NULL,

    cmd_verb        SMALLINT UNSIGNED NOT NULL,
...
CREATE TABLE windows_mdm_command_results (
    ...
    PRIMARY KEY (command_uuid),
    FOREIGN KEY fk_windows_mdm_command_results_command_uuid (command_uuid) REFERENCES windows_mdm_commands (command_uuid) ON DELETE CASCADE ON UPDATE CASCADE
)`)
	if err != nil {
