...
func Up_20260529091823(tx *sql.Tx) error {
  ...
  CONSTRAINT fk_mdm_config_profile_update_settings_exactly_one CHECK ((((IF((apple_declaration_uuid IS NULL),0,1) + IF((windows_profile_uuid IS NULL),0,1))) = 1))
	...
	rows, err := tx.Query("SELECT declaration_uuid, raw_json FROM mdm_apple_declarations;")
	if err != nil {
		return err
	}
	if rows.Err() != nil {
	...
	rows, err = tx.Query("SELECT profile_uuid, syncml FROM mdm_windows_configuration_profiles;")
	if err != nil {
		return err
	}
	if rows.Err() != nil {
