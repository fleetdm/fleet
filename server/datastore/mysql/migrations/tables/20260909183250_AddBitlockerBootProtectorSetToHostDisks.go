...
func Up_20260909183250(tx *sql.Tx) error {
	...
	// running an agent without the reporting query stays in. Only an explicit 0 means the volume has nothing that can
	// unseal it at boot, so no existing host changes disk encryption status because of this column.
