...
`)
	...
	uid, _ := uuid.NewV6()
	r, err := db.Exec(stmt, uid.String(), 0, "com.fleetdm.fleet.mdm.filevault", "Disk encryption", mcBytes)
	...
	uid, _ = uuid.NewV6()
	r, err = db.Exec(stmt, uid.String(), teamID, "com.fleetdm.fleet.mdm.filevault", "Disk encryption", mcBytes)
	require.NoError(t, err)
	...
	stmt = "SELECT identifier, mobileconfig FROM mdm_apple_configuration_profiles WHERE name = ? AND team_id = ?"
	err = db.QueryRow(stmt, "Disk encryption", 0).Scan(&identifier, &mobileconfig)
	require.NoError(t, err)
	require.Equal(t, "com.fleetdm.fleet.mdm.filevault", identifier)
	require.Equal(t, mcBytes, mobileconfig)

	err = db.QueryRow(stmt, "Disk encryption", teamID).Scan(&identifier, &mobileconfig)
	require.NoError(t, err)
	require.Equal(t, "com.fleetdm.fleet.mdm.filevault", identifier)
	require.Equal(t, mcBytes, mobileconfig)
