...
func Up_20260601204606(tx *sql.Tx) error {
	// poll_schedule_relaxed tracks whether the enrollment's DMClient poll schedule has been relaxed because the
	// host's fleetd can be woken on demand (Windows MDM sync). The management session reconciles the schedule
	// against this so it does not re-send the poll Replace on every session.
