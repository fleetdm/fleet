...
func init() {
	MigrationClient.AddMigration(Up_20260818155045, Down_20260818155045)
}

func Up_20260818155045(tx *sql.Tx) error {
	...
	//
	// host_certificate_templates.fleet_challenge holds the same value as
	// challenges.challenge and is joined against it, so it must move in lockstep
	// or the join hits an "illegal mix of collations" error.
	migrations := []struct {
		what string
		stmt string
	}{
...
func Down_20260818155045(tx *sql.Tx) error {
	return nil
}
