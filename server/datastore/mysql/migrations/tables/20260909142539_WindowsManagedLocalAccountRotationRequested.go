...
func init() {
	MigrationClient.AddMigration(Up_20260909142539, Down_20260909142539)
}

// Up_20260909142539 adds mdm_windows_enrollments.managed_local_account_rotation_requested. It lives on the enrollment
// row so the per-poll orbit config read stays a single lookup, and re-enrollment clears a stale request for free.
func Up_20260909142539(tx *sql.Tx) error {
	if _, err := tx.Exec(
...
func Down_20260909142539(tx *sql.Tx) error {
	return nil
}
