...
func TestUp_20260210155109(t *testing.T) {
	db := applyUpToPrev(t)
...
Reasons: PENDING, and USER_ACTION. Other settings are applied.`
	...
	var reverify bool
	err = db.QueryRow(`SELECT can_reverify FROM host_mdm_android_profiles WHERE host_uuid = 'hostid1'`).Scan(&reverify)
	require.False(t, reverify)

	_, err = db.Exec(`UPDATE host_mdm_android_profiles SET can_reverify = 1 WHERE host_uuid = 'hostid1'`)
	require.NoError(t, err)
}
