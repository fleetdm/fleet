...
func Up_20250318165922(tx *sql.Tx) error {
	if columnExists(tx, "mdm_windows_configuration_profiles", "checksum") && columnExists(tx, "host_mdm_windows_profiles", "checksum") {
		return nil
	}
	_, err := tx.Exec(
		`ALTER TABLE mdm_windows_configuration_profiles ADD COLUMN checksum BINARY(16) AS (UNHEX(MD5(syncml))) STORED;
		 ALTER TABLE host_mdm_windows_profiles ADD COLUMN checksum BINARY(16) NOT NULL DEFAULT 0;
