...
func Up_20260525130443(tx *sql.Tx) error {
	...
	`); err != nil {
		return fmt.Errorf("create host_policy_runs table: %w", err)
	}

	// host_policy_runs_to_policy_automation_executions is a joint table relating
	...
	`); err != nil {
		return fmt.Errorf("create host_policy_runs_to_policy_automation_executions table: %w", err)
	}
	...
	// policy reset removes the host_policy_runs row.
	//
