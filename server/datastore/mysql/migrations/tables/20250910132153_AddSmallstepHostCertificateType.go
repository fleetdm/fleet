...
func init() {
	MigrationClient.AddMigration(Up_20250910132153, Down_20250910132153)
}

func Up_20250910132153(tx *sql.Tx) error {
	_, err := tx.Exec(`
...
MODIFY COLUMN type ENUM('digicert','custom_scep_proxy','ndes','smallstep') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'ndes';	`)
	...
	// TODO(sca): do we want to reuse username/password for smallstep SCEP or add new columns?
	_, err = tx.Exec(`
...
func Down_20250910132153(tx *sql.Tx) error {
	return nil
}
