package tables

import (
	"testing"
	"time"

	"github.com/stretchr/testify/require"
)

func TestUp_20260925155809(t *testing.T) {
	db := applyUpToPrev(t)

	// Force several batches so the id cursor has to cross batch boundaries.
	prev := touchWindowsMDMEnrollmentsBatchSize
	touchWindowsMDMEnrollmentsBatchSize = 2
	t.Cleanup(func() { touchWindowsMDMEnrollmentsBatchSize = prev })

	execNoErr(t, db, `
		INSERT INTO hosts (osquery_host_id, node_key, hostname, uuid, platform)
		VALUES ('oh-live', 'nk-live', 'win-live', 'live-uuid', 'windows')`)

	old := time.Now().UTC().Add(-90 * 24 * time.Hour).Truncate(time.Second)
	insert := func(deviceID, hostUUID string) {
		execNoErr(t, db, `
			INSERT INTO mdm_windows_enrollments (
				mdm_device_id, mdm_hardware_id, device_state, device_type, device_name,
				enroll_type, enroll_user_id, enroll_proto_version, enroll_client_version,
				host_uuid, created_at, updated_at)
			VALUES (?, ?, 'enrolled', 'CIMClient_Windows', 'DESKTOP', 'ProgrammaticEnrollment', '', '5.0', '10.0', ?, ?, ?)`,
			deviceID, "hw-"+deviceID, hostUUID, old, old)
	}
	// Every kind of enrollment gets the grace, and five rows with a batch size
	// of 2 means three batches.
	devices := map[string]string{
		"orphaned-1": "deleted-uuid-1",
		"superseded": "live-uuid",
		"orphaned-2": "deleted-uuid-2",
		"unlinked":   "",
		"current":    "live-uuid",
	}
	for _, id := range []string{"orphaned-1", "superseded", "orphaned-2", "unlinked", "current"} {
		insert(id, devices[id])
	}

	applyNext(t, db)

	// A wide margin absorbs clock drift between Go and the MySQL server.
	for id := range devices {
		var ts time.Time
		require.NoError(t, db.Get(&ts, `SELECT updated_at FROM mdm_windows_enrollments WHERE mdm_device_id = ?`, id))
		require.WithinDuration(t, time.Now().UTC(), ts.UTC(), time.Hour, "%s gets a fresh window", id)
	}
}
