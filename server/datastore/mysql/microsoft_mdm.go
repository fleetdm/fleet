...
//
// TEST-ONLY: production never calls this directly. Windows profile
// reconciliation in production is performed by the
// mdm_windows_profile_manager cron (see ReconcileWindowsProfiles), which
// runs the same diff globally every 30s. The test datastore constructor
// (initializeDatabase) installs this method as ds.testWindowsEagerHook so
// existing tests that observe host_mdm_windows_profiles state immediately
// after BulkSetPendingMDMHostProfiles continue to pass without rewriting
// every assertion to drive the cron explicitly. Tests that want to
// exercise the production async path call ds.DisableTestWindowsEagerHook.
