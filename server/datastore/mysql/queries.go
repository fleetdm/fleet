...
func (ds *Datastore) deleteQueryStats(ctx context.Context, queryIDs []uint) {
	...
	if err != nil {
		ds.slogger().ErrorContext(ctx, "error creating delete query stats statement", "err", err)
	} else {
		_, err = ds.writer(ctx).ExecContext(ctx, stmt, args...)
		if err != nil {
			ds.slogger().ErrorContext(ctx, "error deleting query stats", "err", err)
		}
	}
	...
	stmt, args, err = sqlx.In(stmt, queryIDs)
	if err != nil {
		ds.slogger().ErrorContext(ctx, "error creating delete aggregated stats statement", "err", err)
	} else {
		_, err = ds.writer(ctx).ExecContext(ctx, stmt, args...)
		if err != nil {
			ds.slogger().ErrorContext(ctx, "error deleting aggregated stats", "err", err)
		}
	}
}
