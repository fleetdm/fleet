...
WHERE
	...
	if len(inclAny) > 0 && len(exclAny) > 0 {
		...
		ds.slogger().WarnContext(ctx, "vpp app has both include and exclude labels", "vpp_apps_teams_id", app.VPPAppsTeamsID, "include", fmt.Sprintf("%v", inclAny), "exclude", fmt.Sprintf("%v", exclAny))
	}
...
`
	...
	if ds.pusher != nil {
		if _, err := ds.pusher.Push(ctx, []string{hostData.UUID}); err != nil {
			ds.slogger().ErrorContext(ctx, "failed to send push notification", "err", err, "hostID", hostID, "hostUUID", hostData.UUID)
		}
	}
	return nil
}
