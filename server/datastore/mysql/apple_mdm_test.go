...
func expectAndroidProfiles(
	...
	for _, profileUUID := range gotUUIDs {
		decl, err := ds.GetMDMAndroidConfigProfile(ctx, profileUUID)
		require.NoError(t, err)
		got = append(got, decl)
	}
	// create map of expected uuids keyed by identifier
	wantMap := make(map[string]*fleet.MDMAndroidConfigProfile, len(want))
	for _, cp := range want {
		wantMap[cp.ProfileUUID] = cp
	}
	...
	for _, gotA := range got {
		...
		wantA := wantMap[gotA.ProfileUUID]

		m[gotA.ProfileUUID] = gotA.ProfileUUID
		...
		wantBytes, err := JSONRemarshal(wantA.RawJSON)
		require.NoError(t, err)
		...
		require.Equal(t, wantA.Name, gotA.Name)

		// for labels, only care about ID and Name (the exclude, require all
		// fields, etc. are reflected by the field that contains the label)
		require.Equal(t, jsonMustMarshal(wantA.LabelsIncludeAll), jsonMustMarshal(gotA.LabelsIncludeAll))
		require.Equal(t, jsonMustMarshal(wantA.LabelsIncludeAny), jsonMustMarshal(gotA.LabelsIncludeAny))
		require.Equal(t, jsonMustMarshal(wantA.LabelsExcludeAny), jsonMustMarshal(gotA.LabelsExcludeAny))
	}
	return m
}
