...
CREATE TABLE `software_titles` (
  ...
  `additional_identifier` int unsigned GENERATED ALWAYS AS ((case when (`source` = _utf8mb4'ios_apps') then 1 when (`source` = _utf8mb4'ipados_apps') then 2 when (`bundle_identifier` is not null) then 0 else NULL end)) VIRTUAL,
  PRIMARY KEY (`id`),
