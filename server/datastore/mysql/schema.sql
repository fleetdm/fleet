...
CREATE TABLE `software_title_team_pins` (
  ...
  `updated_at` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
  `fleet_maintained_app_id` int unsigned NOT NULL DEFAULT '0',
  PRIMARY KEY (`team_id`,`title_id`,`fleet_maintained_app_id`),
  KEY `fk_pin_title` (`title_id`),
  KEY `fk_pin_fleet_maintained_app` (`fleet_maintained_app_id`),
  CONSTRAINT `fk_pin_fleet_maintained_app` FOREIGN KEY (`fleet_maintained_app_id`) REFERENCES `fleet_maintained_apps` (`id`) ON DELETE CASCADE,
