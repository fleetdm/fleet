...
CREATE TABLE `software_titles` (
  ...
  `bundle_identifier` varchar(255) CHARACTER SET ascii COLLATE ascii_general_ci DEFAULT NULL,
  `additional_identifier` tinyint unsigned GENERATED ALWAYS AS ((case when (`source` = _ascii'ios_apps') then 1 when (`source` = _ascii'ipados_apps') then 2 when (`bundle_identifier` is not null) then 0 else NULL end)) VIRTUAL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `idx_sw_titles` (`name`,`source`,`browser`,`bundle_identifier`),
