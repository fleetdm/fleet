...
func (man Manager) addConfigs() {
	...
	man.addConfigBool("mdm.enable_custom_os_updates_and_filevault", false, "Allow usage of custom OS update and FileVault/BitLocker configuration profiles (may conflict with Fleet-managed profiles)")
	man.addConfigBool("mdm.allow_all_declarations", false, "Allow all MDM declaration types, bypassing safety checks for forbidden types, reserved identifiers, and required prefixes")
