...
func (ds *Datastore) GetHostsForRecoveryLockAction(ctx context.Context) ([]recoverykeypassword.HostNeedingRecoveryLock, error) {
	// Query hosts that:
	...
	hosts := make([]recoverykeypassword.HostNeedingRecoveryLock, len(results))
	for i, r := range results {
		hosts[i] = recoverykeypassword.HostNeedingRecoveryLock{
			HostID:   r.HostID,
			HostUUID: r.HostUUID,
		}
	}
