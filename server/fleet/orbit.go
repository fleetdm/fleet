...
type OrbitConfigNotifications struct {
	...
	// WindowsMDMSyncRequest is set to true when the host's Windows MDM enrollment has queued commands awaiting
	// delivery and the device should start an OMA-DM session now (via deviceenroller) instead of waiting for its
	// next scheduled poll. Only set for hosts whose fleetd advertises CapabilityWindowsMDMSync. Older fleetd that
	// does not understand this field ignores it.
