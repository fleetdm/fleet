...
// OrbitHostNodeKeyer is implemented by orbit request structs that carry a node key.
type OrbitHostNodeKeyer interface {
	// SetOrbitNodeKey is used by clients to set the orbit node key on requests.
	SetOrbitNodeKey(nodeKey string)
	// OrbitHostNodeKey is used the server to extract orbit node key from requests.
	OrbitHostNodeKey() string
}
