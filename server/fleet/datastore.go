...
type Datastore interface {
	...
	// ingestion don't clobber each other's view. observedScopes further limits
	// reconciliation to the (source, username) scopes the agent could
	// authoritatively enumerate this run; a nil slice means every scope was
	// observed (macOS keychains and the MDM path), while a non-nil slice (Windows)
	// preserves certificates for scopes it could not see, e.g. a logged-off user.
