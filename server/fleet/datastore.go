...
type Datastore interface {
	...
	// SetOrUpdateEntraJoinHostDeviceMapping records the UPN from the device's
	// Entra join record as the host's IdP username and links the matching SCIM
	// user. Only SCIM-provisioned UPNs on hosts without a Fleet MDM enrollment are
	// mapped; an unknown or empty UPN, or a Fleet MDM enrollment, removes a previous
	// Entra join mapping. Manual ("idp") and authenticated ("mdm_idp_accounts")
	// mappings win and make this a no-op. Returns true when the mapping was
