...
const (
	...
	// OneTimeChallengeTTL is the time to live for one-time challenges. The
	// challenge is generated at profile-render time but consumed when the
	// device makes its SCEP request, which can be hours or days later for
	// offline devices (sleeping laptops, devices on a plane). 7 days covers
	// the typical offline window. Once consumed, the challenge is deleted
