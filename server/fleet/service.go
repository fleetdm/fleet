...
type Service interface {
	...
	// GetPKICertificates returns the list of PKI certificates (only metadata).
	GetPKICertificates(ctx context.Context) ([]PKICertificate, error)
	// GetCertCSR returns a CSR as base64 encoded bytes.
	GetCertCSR(ctx context.Context, name string) ([]byte, error)
	// UploadCert uploads a certificate to Fleet that should have been generated by the downloaded CSR.
	UploadCert(ctx context.Context, name string, cert io.ReadSeeker) error
	// DeleteCert deletes the certificate uploaded to Fleet.
