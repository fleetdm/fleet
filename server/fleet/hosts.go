...
type HostMDMManagedLocalAccount struct {
	...
	// Detail carries the device-reported reason the account could not be created,
	// for accounts created by fleetd (Windows). Empty for macOS accounts, which are
	// configured by an MDM command instead.
	Detail string `json:"detail" db:"-" csv:"-"`
