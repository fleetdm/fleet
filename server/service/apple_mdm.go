...
const (
	...
	SameProfileNameEditMacErrorMsg = "Couldn't edit macos_settings.custom_settings. More than one configuration profile have the same name (PayloadDisplayName for .mobileconfig and file name for .json and .xml)."
)
...
func (svc *Service) BatchSetMDMAppleProfiles(ctx context.Context, tmID *uint, tmName *string, profiles [][]byte, dryRun, skipBulkPending bool) error {
	...
	for i, prof := range profiles {
		...
		if byName[mdmProf.Name] {
			return ctxerr.Wrap(ctx,
				fleet.NewInvalidArgumentError(fmt.Sprintf("profiles[%d]", i), SameProfileNameEditMacErrorMsg),
	...
	if !skipBulkPending {
		...
		for _, p := range allProfs {
			if byName[p.Name] {
				...
				default:
					err := fleet.NewInvalidArgumentError("PayloadDisplayName", SameProfileNameEditMacErrorMsg)
