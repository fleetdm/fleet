...
func verifyLabelsToAssociate(ctx context.Context, ds fleet.Datastore, entityTeamID *uint, labelNames []string) error {
	...
	labels, err := loadLabelsFromNames(ctx, ds, labelNames)
	if err != nil {
		return ctxerr.Wrap(ctx, err, "labels by name")
	}
	if entityTeamID == nil || *entityTeamID == 0 {
		// teamID == nil: global entity (like "All teams" policies and "All team" queries)
		// teamID == 0: "no team" entity.
	...
	for _, label := range labels {
		...
		// Team entities can reference labels that belong to the same team.
