...
// ///////////////////////////////////////////////////////////////////////////////
// Report Orbit errors to Fleet
// ///////////////////////////////////////////////////////////////////////////////

type orbitErrorReportRequest struct {
	fleet.OrbitErrorReport
	OrbitNodeKey string `json:"orbit_node_key"`
}

// interface implementation required by the OrbitClient
func (r *orbitErrorReportRequest) setOrbitNodeKey(nodeKey string) {
	r.OrbitNodeKey = nodeKey
}

// interface implementation required by orbit authentication
func (r *orbitErrorReportRequest) orbitHostNodeKey() string {
	return r.OrbitNodeKey
}

type orbitErrorReportResponse struct {
	Err error `json:"error,omitempty"`
}

func (r orbitErrorReportResponse) error() error { return r.Err }

func postOrbitErrorReportEndpoint(ctx context.Context, request interface{}, svc fleet.Service) (errorer, error) {
	req := request.(*orbitErrorReportRequest)
	svc.ReportOrbitError(ctx, req.OrbitErrorReport)
	return orbitErrorReportResponse{}, nil
}

func (svc *Service) ReportOrbitError(ctx context.Context, report fleet.OrbitErrorReport) {
	// this is not a user-authenticated endpoint
	svc.authz.SkipAuthorization(ctx)
	ctxerr.HandleOrbitError(ctx, report)
}
