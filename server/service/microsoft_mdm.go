...
// clientTokenError marks a binary-security-token failure caused by the
// request's own credentials (missing, malformed, unknown, or ineligible)
// rather than by Fleet's backend; its message is written for the caller and
// survives into the SOAP fault.
type clientTokenError struct{ err error }

func newClientTokenError(msg string) error { return clientTokenError{errors.New(msg)} }
func (e clientTokenError) Error() string   { return e.err.Error() }
func (e clientTokenError) Unwrap() error   { return e.err }
...
func mdmMicrosoftPolicyEndpoint(ctx context.Context, request interface{}, svc fleet.Service) (mdm_types.Errorer, error) {
	...
	// Getting the GetPoliciesResponse message. Failures caused by the request's
	// own token keep their reasons; backend failures get a generic one, with
	// the detail already recorded in the request log.
	...
	if err != nil {
		...
		if !errors.As(err, &cte) && !errors.As(err, &iae) {
			hideSoapFaultDetail(soapFault)
		}
		return getSoapResponseFault(req.GetMessageID(), soapFault), nil
	}
...
func (svc *Service) authBinarySecurityToken(ctx context.Context, authToken *fleet.HeaderBinarySecurityToken) (claim string, hostUUID string, enrollType fleet.WindowsMDMEnrollType, err error) {
	...
	if authToken == nil {
		return "", "", 0, newClientTokenError("authToken is empty")
	}

	err = authToken.IsValidToken()
	if err != nil {
		return "", "", 0, newClientTokenError("authToken is not valid")
	}
	...
	if authToken.IsDeviceToken() {
		...
		if err != nil {
			return "", "", 0, clientTokenError{fmt.Errorf("token creation error %v", err)}
		}

		// Validating the Binary Security Token Payload
		err = binSecToken.IsValidToken()
		if err != nil {
			return "", "", 0, clientTokenError{fmt.Errorf("invalid token data %v", err)}
		}
		...
		if binSecToken.Type == mdm_types.WindowsMDMProgrammaticEnrollmentType {
			...
			switch {
			case fleet.IsNotFound(err) || (err == nil && host == nil):
				return "", "", 0, newClientTokenError("host not found for orbit node key")
			case err != nil:
			...
			if !isEligibleForWindowsMDMEnrollment(host, mdmInfo) {
				return "", "", 0, newClientTokenError("host is not elegible for Windows MDM enrollment")
			}
		...
		if binSecToken.Type == mdm_types.WindowsMDMAutomaticEnrollmentType {
			...
			if err != nil {
				return "", "", 0, clientTokenError{ctxerr.Wrap(ctx, err, "issue retrieving UPN from Auth token")}
			}
	...
	if authToken.IsAzureJWTToken() {
		...
		if err != nil {
			return "", "", 0, clientTokenError{fmt.Errorf("binary security token claim failed: %v", err)}
		}
		...
		if !hasAuthorizedAzureAudience(tokenData.Audience, expectedURLParsed.Host, appConfig.MDM.WindowsEntraClientIDs.Value) {
			...
			return "", "", 0, clientTokenError{ctxerr.Errorf(ctx, "token audience is not authorized")}
		}
		if !hasAuthorizedAzureTenant(entraTenantIDs, tokenData.TenantID) {
			...
			return "", "", 0, clientTokenError{ctxerr.New(ctx, "token tenant is not authorized")}
		}
	...
	return "", "", 0, clientTokenError{ctxerr.New(ctx, "token is not authorized")}
}
