...
func (svc *Service) authBinarySecurityToken(ctx context.Context, authToken *fleet.HeaderBinarySecurityToken) (claims microsoft_mdm.AzureData, hostUUID string, err error) {
	if authToken == nil {
		return microsoft_mdm.AzureData{}, "", errors.New("authToken is empty")
	}

	err = authToken.IsValidToken()
	if err != nil {
		return microsoft_mdm.AzureData{}, "", errors.New("authToken is not valid")
	}
	...
	if authToken.IsDeviceToken() {
		...
		if err != nil {
			return microsoft_mdm.AzureData{}, "", fmt.Errorf("token creation error %v", err)
		}

		// Validating the Binary Security Token Payload
		err = binSecToken.IsValidToken()
		if err != nil {
			return microsoft_mdm.AzureData{}, "", fmt.Errorf("invalid token data %v", err)
		}
		...
		if binSecToken.Type == mdm_types.WindowsMDMProgrammaticEnrollmentType {
			...
			if err != nil {
				return microsoft_mdm.AzureData{}, "", fmt.Errorf("host data cannot be found %v", err)
			}

			mdmInfo, err := svc.ds.GetHostMDM(ctx, host.ID)
			if err != nil && !errors.Is(err, sql.ErrNoRows) {
				return microsoft_mdm.AzureData{}, "", errors.New("unable to retrieve host mdm info")
			}
			...
			if !isEligibleForWindowsMDMEnrollment(host, mdmInfo) {
				return microsoft_mdm.AzureData{}, "", errors.New("host is not elegible for Windows MDM enrollment")
			}

			// No errors, token is authorized
			return microsoft_mdm.AzureData{
				UPN: binSecToken.Payload.OrbitNodeKey,
			}, host.UUID, nil
		}
		...
		if binSecToken.Type == mdm_types.WindowsMDMAutomaticEnrollmentType {
			...
			// TODO: Should we also capture given name and family name here, if possible?
			upnToken, err := svc.wstepCertManager.GetSTSAuthTokenUPNClaim(binSecToken.Payload.AuthToken)
			if err != nil {
				return microsoft_mdm.AzureData{}, "", ctxerr.Wrap(ctx, err, "issue retrieving UPN from Auth token")
			}
	...
	if authToken.IsAzureJWTToken() {
		...
		if err != nil {
			return microsoft_mdm.AzureData{}, "", fmt.Errorf("binary security token claim failed: %v", err)
		}
	...
	return microsoft_mdm.AzureData{}, "", errors.New("token is not authorized")
}
...
func (svc *Service) GetMDMWindowsEnrollResponse(ctx context.Context, secTokenMsg *fleet.RequestSecurityToken, authToken *fleet.HeaderBinarySecurityToken) (*fleet.RequestSecurityTokenResponseCollection, error) {
	...
	tokenData, hostUUID, err := svc.authBinarySecurityToken(ctx, authToken)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "validate binary security token")
	}
	...
	err = svc.storeWindowsMDMEnrolledDevice(ctx, tokenData, hostUUID, secTokenMsg, credentialsHash)
	if err != nil {
...
func (svc *Service) storeWindowsMDMEnrolledDevice(ctx context.Context, tokenData microsoft_mdm.AzureData, hostUUID string, secTokenMsg *fleet.RequestSecurityToken, credentialsHash []byte) error {
	const (
	...
	enrolledDevice := &fleet.MDMWindowsEnrolledDevice{
		...
		MDMEnrollUserID:         tokenData.UPN, // This could be Host UUID or UPN email
	...
	if !enrolledDevice.MDMNotInOOBE && microsoft_mdm.IsValidUPN(tokenData.UPN) {
		...
		err = svc.ds.InsertMDMIdPAccount(ctx, &mdm_types.MDMIdPAccount{
			...
			Email:    tokenData.UPN,
			Username: strings.Split(tokenData.UPN, "@")[0],
			Fullname: fmt.Sprintf(tokenData.GivenName + " " + tokenData.FamilyName),
		})
		if err != nil {
			return fmt.Errorf("%s: failed to insert MDM IDP account for user %s: %w", error_tag, tokenData.UPN, err)
		}
	}
