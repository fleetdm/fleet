...
// The caName parameter identifies the specific NDES CA instance (e.g., "NDES", "NDES_OKTA_PROD").
func ndesChallengeErrorToDetail(err error, caName string) string {
	varName := "$FLEET_VAR_" + string(fleet.FleetVarNDESSCEPChallengePrefix) + caName
	switch {
...
func ReconcileWindowsProfiles(ctx context.Context, ds fleet.Datastore, logger *slog.Logger) error {
	...
	deps := microsoft_mdm.ProfilePreprocessDependencies{
		...
		NDESCAs:                    groupedCAs.ToNDESSCEPProxyCAMap(),
