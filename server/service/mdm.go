...
func getAppleProfiles(
	...
	if !appCfg.MDM.EnabledAndConfigured {
		...
		return nil, nil, ctxerr.Wrap(ctx, fleet.NewInvalidArgumentError("mdm", "cannot set custom settings: Apple MDM is not configured"))
	}
...
func getWindowsProfiles(
	...
	if !appCfg.MDM.WindowsEnabledAndConfigured {
		...
		return nil, ctxerr.Wrap(ctx, fleet.NewInvalidArgumentError("mdm", "cannot set custom settings: Windows MDM is not configured"))
	}
