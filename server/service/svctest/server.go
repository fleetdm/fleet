...
func RunServerForTestsWithServiceWithDS(t *testing.T, ctx context.Context, ds fleet.Datastore, svc fleet.Service,
	...
	if len(opts) > 0 {
		...
		if mdmStorage != nil && scepStorage != nil {
			...
			checkInAndCommand := service.NewMDMAppleCheckinAndCommandService(ds, commander, vppInstaller, opts[0].License.IsPremium(), logger, redis_key_value.New(redisPool), svc.NewActivity, &fleet_mock.MockNotificationsService{})
