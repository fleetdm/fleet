...
func (s *integrationTestSuite) TestListActivities() {
	...
	_, err = s.ds.NewActivity(ctx, &u, fleet.ActivityTypeAppliedSpecPack{}, nil, timestamp)
	require.NoError(t, err)

	_, err = s.ds.NewActivity(ctx, &u, fleet.ActivityTypeDeletedPack{}, nil, timestamp)
	require.NoError(t, err)

	_, err = s.ds.NewActivity(ctx, &u, fleet.ActivityTypeEditedPack{}, nil, timestamp)
	require.NoError(t, err)
