...
func TestMDMAppleReconcileAppleProfiles(t *testing.T) {
	...
	t.Run("replace $FLEET_VAR_"+string(fleet.FleetVarNDESSCEPProxyURLPrefix)+"NDES", func(t *testing.T) {
		var upsertCount int
		failedCall = false
		failedCheck = func(payload []*fleet.MDMAppleBulkUpsertHostProfilePayload) {
			upsertCount++
		...
		enqueueFailForOp = ""
		newContents := "$FLEET_VAR_" + string(fleet.FleetVarNDESSCEPProxyURLPrefix) + "NDES"
