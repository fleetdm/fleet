...
func (s *integrationMDMTestSuite) TestVPPApps() {
	...
	t.Run("vpp apps with labels", func(t *testing.T) {
		...
		activityData = `{"team_name": "%s", "software_title": "%s", "display_name":"", "app_store_id": "%s", "software_icon_url": "https://example.com/images/2", "team_id": %d, "software_title_id": %d, "platform": "%s", "self_service": false, "labels_include_any": [{"id": %d, "name": %q}]}`
