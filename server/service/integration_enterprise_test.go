...
func (s *integrationEnterpriseTestSuite) TestPolicyAutomationResetOnModification() {
	t := s.T()
	ctx := context.Background()
	...
	_, err = s.ds.UpdateScriptContents(ctx, script.ID, "echo 'test v2'")
	require.NoError(t, err)
	...
	type result struct {
		...
		AttemptNumber sql.NullInt64 `db:"attempt_number"`
	...
	require.True(t, results[0].AttemptNumber.Valid)
	require.Equal(t, int64(0), results[0].AttemptNumber.Int64)
	require.False(t, results[0].Canceled)

	// pending, reset to 0, canceled
	require.Equal(t, "exec-2", results[1].ExecutionID)
	require.True(t, results[1].AttemptNumber.Valid)
	require.Equal(t, int64(0), results[1].AttemptNumber.Int64)
	...
	// Create another policy for software install
	policy2, err := s.ds.NewTeamPolicy(ctx, team.ID, nil, fleet.PolicyPayload{
		Name:     t.Name() + "-2",
		Query:    "SELECT 1;",
		Platform: "darwin",
	})
	...
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		_, err := q.ExecContext(ctx, `
			INSERT INTO host_script_results (host_id, execution_id, script_content_id, output, exit_code, script_id, policy_id, attempt_number)
			VALUES (1, 'policy-script-1', ?, 'output', 1, ?, ?, 1)
		`, scriptContentID, script.ID, policy2.ID)
		return err
	})
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		_, err := q.ExecContext(ctx, `
			INSERT INTO host_script_results (host_id, execution_id, script_content_id, output, exit_code, script_id, policy_id, attempt_number)
			VALUES (1, 'policy-script-2', ?, '', NULL, ?, ?, 2)
		`, scriptContentID, script.ID, policy2.ID)
		return err
	})

	// Modify the policy - this should reset attempt_number to 0 for all automations using this policy
	err = s.ds.SavePolicy(ctx, policy2, false, false)
	require.NoError(t, err)
	...
	require.True(t, installResults[0].AttemptNumber.Valid)
	require.Equal(t, int64(0), installResults[0].AttemptNumber.Int64)
	require.True(t, installResults[1].AttemptNumber.Valid)
	require.Equal(t, int64(0), installResults[1].AttemptNumber.Int64)

	// Verify script execution attempts were also reset
	var policyScriptResults []result
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		return sqlx.SelectContext(ctx, q, &policyScriptResults, `
			SELECT execution_id, exit_code, attempt_number, canceled
			FROM host_script_results
			WHERE script_id = ? AND policy_id = ?
			ORDER BY execution_id ASC
		`, script.ID, policy2.ID)
	})

	require.Len(t, policyScriptResults, 2)
	require.Equal(t, "policy-script-1", policyScriptResults[0].ExecutionID)
	require.True(t, policyScriptResults[0].AttemptNumber.Valid)
	require.Equal(t, int64(0), policyScriptResults[0].AttemptNumber.Int64)
	require.Equal(t, "policy-script-2", policyScriptResults[1].ExecutionID)
	require.True(t, policyScriptResults[1].AttemptNumber.Valid)
	require.Equal(t, int64(0), policyScriptResults[1].AttemptNumber.Int64)
}
