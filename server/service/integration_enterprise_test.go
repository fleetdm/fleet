...
func (s *integrationEnterpriseTestSuite) TestCertificatesSpecs() {
	...
	// This request should return 404 if the certificate variable replacement fails
	s.DoRawWithHeaders("GET", fmt.Sprintf("/api/fleetd/certificates/%d", certID), nil, http.StatusNotFound, map[string]string{
		"Authorization": fmt.Sprintf("Node key %s", orbitNodeKey),
	})

	// Add an IDP user for the host
	// This should re-queue the android certificate managed config
	// and allow the variable replacement to succeed
	var putDeviceMappingResp putHostDeviceMappingResponse
	s.DoJSON("PUT", fmt.Sprintf("/api/latest/fleet/hosts/%d/device_mapping", host.ID), putHostDeviceMappingRequest{Email: "foo@example.com", Source: "idp"}, http.StatusOK, &putDeviceMappingResp)

	// Set status to delivered, mimicking the android certificate cron
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		_, err := q.ExecContext(ctx,
			`UPDATE host_certificate_templates SET status = ? WHERE host_uuid = ? AND certificate_template_id = ?`,
			fleet.CertificateTemplateDelivered, host.UUID, certID)
		return err
	})
	...
	assert.Contains(t, getCertResp.Certificate.SubjectName, "foo@example.com")
