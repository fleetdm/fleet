...
func (s *integrationEnterpriseTestSuite) TestNoTeamTicketIntegrationConfig() {
	t := s.T()

	// Set environment variable to use test Zendesk client
	t.Setenv("TEST_ZENDESK_CLIENT", "true")

	// create a test http server to act as the Jira and Zendesk server
	srvURL := startExternalServiceWebServer(t)

	// First, set up global Jira and Zendesk integrations that teams can reference
	// Use the API endpoint to properly configure global integrations
	s.DoRaw("PATCH", "/api/v1/fleet/config", []byte(fmt.Sprintf(`{
		"integrations": {
			"jira": [
				{
	...
	}`, srvURL, srvURL)), http.StatusOK)

	var tmResp teamResponse

	// Test configuring Jira integration for "No Team"
	s.DoJSON("PATCH", "/api/latest/fleet/teams/0", fleet.TeamPayload{
		Integrations: &fleet.TeamIntegrations{
			Jira: []*fleet.TeamJiraIntegration{
				{
					URL:                   srvURL,
	...
	require.Equal(t, fleet.ReservedNameNoTeam, tmResp.Team.Name)
	require.Len(t, tmResp.Team.Config.Integrations.Jira, 1)
	require.Equal(t, srvURL, tmResp.Team.Config.Integrations.Jira[0].URL)
	require.Equal(t, "qux", tmResp.Team.Config.Integrations.Jira[0].ProjectKey)
	require.True(t, tmResp.Team.Config.Integrations.Jira[0].EnableFailingPolicies)
	...
	// Test that we cannot enable both webhook and Jira integration
	s.DoJSON("PATCH", "/api/latest/fleet/teams/0", fleet.TeamPayload{
		WebhookSettings: &fleet.TeamWebhookSettings{
			FailingPoliciesWebhook: fleet.FailingPoliciesWebhookSettings{
				Enable:         true,
	...
	// Clear Jira integration
	s.DoJSON("PATCH", "/api/latest/fleet/teams/0", fleet.TeamPayload{
		Integrations: &fleet.TeamIntegrations{
			Jira: []*fleet.TeamJiraIntegration{},
		},
	}, http.StatusOK, &tmResp)
	require.Empty(t, tmResp.Team.Config.Integrations.Jira)

	// Test configuring Zendesk integration for "No Team"
	s.DoJSON("PATCH", "/api/latest/fleet/teams/0", fleet.TeamPayload{
		Integrations: &fleet.TeamIntegrations{
			Zendesk: []*fleet.TeamZendeskIntegration{
				{
	...
	require.Len(t, tmResp.Team.Config.Integrations.Zendesk, 1)
	require.Equal(t, srvURL, tmResp.Team.Config.Integrations.Zendesk[0].URL)
	require.Equal(t, int64(122), tmResp.Team.Config.Integrations.Zendesk[0].GroupID)
	require.True(t, tmResp.Team.Config.Integrations.Zendesk[0].EnableFailingPolicies)

	// Test that we cannot reference a non-existent global integration
	s.DoJSON("PATCH", "/api/latest/fleet/teams/0", fleet.TeamPayload{
		Integrations: &fleet.TeamIntegrations{
			Jira: []*fleet.TeamJiraIntegration{
				{
	...
	require.Empty(t, tmResp.Team.Config.Integrations.Jira)
	require.Empty(t, tmResp.Team.Config.Integrations.Zendesk)
}
