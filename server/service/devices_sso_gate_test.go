...
func TestExtractDeviceSSOSessionFromCookie(t *testing.T) {
	t.Run("no cookie", func(t *testing.T) {
		r := httptest.NewRequest("GET", "/api/latest/fleet/device/tok", nil)
		_, ok := devicesso.FromContext(extractDeviceSSOSessionFromCookie(t.Context(), r))
		require.False(t, ok)
	})

	t.Run("cookie present", func(t *testing.T) {
		r := httptest.NewRequest("GET", "/api/latest/fleet/device/tok", nil)
		r.AddCookie(&http.Cookie{Name: cookieNameDeviceSSOSession, Value: "session-id"})
		sessionID, ok := devicesso.FromContext(extractDeviceSSOSessionFromCookie(t.Context(), r))
		require.True(t, ok)
