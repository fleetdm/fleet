...
func attachFleetAPIRoutes(r *mux.Router, svc fleet.Service, config config.FleetConfig,
	...
	demdm.WithCustomMiddleware(errorLimiter.Limit("post_device_trigger_linux_key_escrow", desktopQuota)).POST("/api/_version_/fleet/device/{token}/mdm/linux/trigger_key_escrow", triggerLinuxKeyEscrowEndpoint, triggerLinuxKeyEscrowRequest{})
