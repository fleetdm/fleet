...
func TestInstalledApplicationListHandler(t *testing.T) {
	...
	setupMockDS := func(t *testing.T) *mock.DataStore {
		...
		ds.NewActivityFunc = func(_ context.Context, _ *fleet.User, _ fleet.ActivityDetails, _ []byte, _ time.Time) error {
			return nil
		}
	...
	t.Run("app installed with matching version is verified", func(t *testing.T) {
		...
		handler := NewInstalledApplicationListResultsHandler(ds, nil, logger, verifyTimeout, verifyRequestDelay)
