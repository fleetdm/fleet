...
// endpointMaker(svc, "NewGlobalPolicy", &globalPolicyRequest{}, opts)
func endpointMaker(svc fleet.Service, method string, iface interface{}, opts []kithttp.ServerOption) http.Handler {
	// TODO: check that method belongs to svc and that it accepts the
	// TODO: move this to svc to make this shorter?
	// TODO: add response iface and set Err or the only parameter as part of the response, maybe with a tag?
	// TODO: make the call have each of the parameters from the struct, separately... hmm, what about order?
	endpointFunc := func(ctx context.Context, request interface{}, svc fleet.Service) (interface{}, error) {
		castVar := reflect.ValueOf(request)
		destType := reflect.ValueOf(iface).Type()
		parameter := castVar.Convert(destType)
		svcV := reflect.ValueOf(svc)
		svcMethod := svcV.MethodByName(method)
		result := svcMethod.Call([]reflect.Value{parameter})
		if len(result) != 2 {
	...
	return newServer(makeAuthenticatedServiceEndpoint(svc, endpointFunc), makeDecoder(iface), opts)
}
