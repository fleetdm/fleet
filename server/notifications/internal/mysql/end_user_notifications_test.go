...
func testDeleteExpiredEndUserNotifications(t *testing.T, env *testEnv) {
	...
	// past retention and displayed, which no sweep moves off dispatched
...
func testFailEndUserNotificationsForHost(t *testing.T, env *testEnv) {
	...
	t.Run("a wiped host gives up on the notifications it had not delivered", func(t *testing.T) {
		...
		require.NoError(t, env.ds.FailEndUserNotificationsForHost(ctx, hostID, api.EndUserNotificationReasonCanceled))
		...
		assert.Equal(t, api.EndUserNotificationReasonCanceled, *gotPending.LastReason)
		...
		assert.Equal(t, api.EndUserNotificationReasonCanceled, *gotDispatched.LastReason)
	...
	t.Run("an acted notification is left alone", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, api.EndUserNotificationActed, got.Status, "a terminal status records what happened")
		assert.Nil(t, got.LastReason)
	})
	...
	t.Run("another host's notifications are not touched", func(t *testing.T) {
		...
		require.NoError(t, env.ds.FailEndUserNotificationsForHost(ctx, wipedHostID, api.EndUserNotificationReasonCanceled))
