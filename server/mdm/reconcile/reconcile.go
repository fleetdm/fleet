// Package reconcile holds the platform-neutral core of MDM profile
// reconciliation: the include/exclude label handlers and the team+label
// applicability dispatcher. Every platform's batched reconciler (Apple
// profiles, Apple DDM declarations, Windows profiles) routes through this
// package so team / label semantics cannot drift between platforms.
//
// Platform eligibility is intentionally NOT part of this package: each
// caller applies its own platform gate (e.g. apple_mdm.IsEligiblePlatform)
// before, or together with, EntityAppliesToHost.
package reconcile
...
// EntityAppliesToHost is the shared top-level dispatcher for MDM
// label-gated entities (profiles and declarations). It applies the team
// gate, then composes the include + exclude label handlers carried by the
// entity.
//
// hostEffectiveTeamID is the host's team with nil normalized to 0
// (team_id=0 is its own "no team" scope, not a fallback for teamed
// hosts). hostLabelUpdatedAt is the host's labels-last-scanned timestamp,
// used by the exclude-any handler's dynamic-label timing rule. hostLabels
// is the set of label IDs the host is a member of.
...
//
// Returns true if the host should be excluded, false if the host passes
// the exclude gate.
