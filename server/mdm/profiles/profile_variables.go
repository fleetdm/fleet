...
// BuildNDESSCEPProxyURL constructs the NDES SCEP proxy URL for a given host, profile, and CA name.
func BuildNDESSCEPProxyURL(mdmURL string, hostUUID string, profileUUID string, caName string) string {
	return fmt.Sprintf("%s%s%s", mdmURL, SCEPProxyPath,
		url.PathEscape(fmt.Sprintf("%s,%s,%s", hostUUID, profileUUID, caName)))
}
