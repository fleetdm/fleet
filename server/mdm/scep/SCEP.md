...
When the Fleet challenge is invalid (already consumed or expired), Fleet automatically requeues:

1. `ResendHostCertificateProfile()` sets profile `status = NULL`
2. Cron job finds profiles where `status IS NULL`
3. `NewChallenge()` generates a fresh Fleet challenge
4. Profile is re-delivered with new SCEP URL containing the new Fleet challenge
