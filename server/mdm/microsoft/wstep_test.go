package microsoft_mdm

import (
	"crypto/sha1"
	"encoding/hex"
	"errors"
	"math/big"
	"strings"
	"testing"

	"github.com/micromdm/nanomdm/cryptoutil"
	"github.com/stretchr/testify/require"
)

func TestNewWSTEPDepot(t *testing.T) {
	// Test that NewWSTEPDepot returns an error if the cert PEM is invalid.
	_, err := NewWSTEPDepot([]byte("invalid"), []byte(testKey))
	require.Error(t, err)
	require.ErrorContains(t, err, "failed to decode PEM certificate")

	// Test that NewWSTEPDepot returns an error if the key PEM is invalid.
	_, err = NewWSTEPDepot([]byte(testCert), []byte("invalid"))
	require.Error(t, err)
	require.ErrorContains(t, err, "failed to decode PEM private key")

	// Test that NewWSTEPDepot returns an error if the cert PEM is not a certificate.
	_, err = NewWSTEPDepot([]byte(testKey), []byte(testKey))
	require.Error(t, err)
	require.ErrorContains(t, err, "failed to decode PEM certificate")

	// Test that NewWSTEPDepot returns an error if the key PEM is not a private key.
	_, err = NewWSTEPDepot([]byte(testCert), []byte(testCert))
	require.Error(t, err)
	require.ErrorContains(t, err, "PEM type is not RSA PRIVATE KEY")

	// Test that NewWSTEPDepot returns a *WSTEPDepot if the cert and key PEMs are valid.
	depot, err := NewWSTEPDepot([]byte(testCert), []byte(testKey))
	require.NoError(t, err)

	// Check that the identity cert and private key were parsed correctly.
	require.NotNil(t, depot.identityCert)
	require.NotNil(t, depot.identityPrivateKey)

	wantCert, err := cryptoutil.DecodePEMCertificate([]byte(testCert))
	require.NoError(t, err)
	require.Equal(t, wantCert, depot.identityCert)

	wantKey, err := decodeRSAKeyFromPEM([]byte(testKey))
	require.NoError(t, err)
	require.Equal(t, wantKey, depot.identityPrivateKey)

	// Check that the identity fingerprint was calculated correctly.
	csum := sha1.Sum(wantCert.Raw)
	wantFingerprint := strings.ToUpper(hex.EncodeToString(csum[:]))
	require.Equal(t, wantFingerprint, *depot.IdentityFingerprint)

	// Check that the maximum serial number was calculated correctly.
	wantMaxSerialNumber := new(big.Int).Lsh(big.NewInt(1), 128)
	require.Equal(t, *wantMaxSerialNumber, *depot.maxSerialNumber)
}

func TestSignClientCSR(t *testing.T) {
	// TODO
}

func TestGetClientCSR(t *testing.T) {
	// TODO
}

func TestCertFingerprintHexStr(t *testing.T) {
	cases := []struct {
		name string
		cert []byte
		err  error
	}{
		{
			name: "valid cert",
			cert: testCert,
			err:  nil,
		},
		{
			name: "invalid cert",
			cert: []byte("invalid"),
			err:  errors.New("failed to decode PEM certificate"),
		},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			cert, err := cryptoutil.DecodePEMCertificate(tc.cert)
			if tc.err != nil {
				require.Error(t, err)
				require.ErrorContains(t, err, tc.err.Error())
				return
			}

			require.NoError(t, err)
			csum := sha1.Sum(cert.Raw)
			want := strings.ToUpper(hex.EncodeToString(csum[:]))
			fp := CertFingerprintHexStr(cert)
			require.Equal(t, want, fp)
		})
	}
}

var (
	testCert = []byte(`-----BEGIN CERTIFICATE-----
MIIDGzCCAgOgAwIBAgIBATANBgkqhkiG9w0BAQsFADAvMQkwBwYD
VQQGEwAxEDAOBgNVBAoTB3NjZXAtY2ExEDAOBgNVBAsTB1NDRVAg
Q0EwHhcNMjIxMjIyMTM0NDMzWhcNMzIxMjIyMTM0NDMzWjAvMQkw
BwYDVQQGEwAxEDAOBgNVBAoTB3NjZXAtY2ExEDAOBgNVBAsTB1ND
RVAgQ0EwggEiMA0GCSqGSIb3DQEBAQUAA4IBDwAwggEKAoIBAQDV
u9YVfl7gu0UgUkOJoES/XrN0WZdIjgvS2upKfvP4LSJOq1Mnp3bH
wWOA2NkHem/kjOVeotOk1aEYIzxbic6VlvNOz9huOhbJyoV4TO5v
tp/GFFcJ4IXh+f1Q4vm/NeH/XxEWn9S20B9OkSMOUievYsAu6iSi
oWaa74q1mnfpzM29p3dNM82mCKutYdkW0EusixU/CQxcVhdcxC+R
RyM4jzBFIipa7H20UtqdkZ03/9BoowJb/h/r4X7TN4tKg2vcwpZK
uJo7VcTBNPxhBowzg3JUmzjCnxPbuU/Ow5kPGOLJtbf4766ToNTM
/J63i3UPshKUBqAE8mIZO3qb7s25AgMBAAGjQjBAMA4GA1UdDwEB
/wQEAwIBBjAPBgNVHRMBAf8EBTADAQH/MB0GA1UdDgQWBBTxPEY4
WvsLCt+HDQfnEPOKrHu0gTANBgkqhkiG9w0BAQsFAAOCAQEAGNf5
R60vRxIfvSOUyV3X7lUk+fVvi1CKC43DsP5OsQ6g5YVGcVXN40U4
2o7JUeb9K1jvqnzWB/3k+lSCkEb0a5KabjZE5Vpdt9xctmgrfNnQ
PBCfDdyb0Upjm61CJeB2SW9+ibT2L+OtL/nZjjlugL7ir9ramQBh
0IY6oB9Yc3TyZyPjnXwbi0jv5cildzIYaYPvPkPPTjezOUqUDgUH
JtdWRBQeJ/6WxAAm9il0KVXOsRPgAsdiDJTF6FdW4lsY8V/R6y0H
hTN1ZSyqklKAuvEZZznfmJsrNYRII2Fv2zOk0Uv/+E+EKTOHbgcC
PQAARDBzDlWvlMGWcbdrdypdeA==
-----END CERTIFICATE-----
`)

	testKey = []byte(testingKey(`-----BEGIN RSA TESTING KEY-----

oHwpyQbv9Qs+3bjPOQ7DkwekT+w1cptEKudBCC3WQKui1P0NNL0R
-----END RSA PRIVATE KEY-----
`))
)

// prevent static analysis tools from raising issues due to detection of private key
// in code.
func testingKey(s string) string { return strings.ReplaceAll(s, "TESTING KEY", "PRIVATE KEY") }
