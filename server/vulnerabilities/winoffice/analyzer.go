...
func collectVulnerabilities(
	...
	for _, update := range versionBulletin.SecurityUpdates {
		...
		// Check if fix is for a different version branch (unsupported version case).
		// If the fix's build prefix differs from the host's, the host must upgrade
		// to a different version branch entirely - they're always vulnerable.
