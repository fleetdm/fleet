...
func TestMatchesOperatingSystem(t *testing.T) {
	...
	tc := []struct {
		...
		// the fixture's HLK products reduce to the same name as the real ones.
		{
	...
	// Each case is repeated so that a choice depending on map iteration order
	// shows up. Matching a product that carries no CVEs makes the analyzer treat
	// every CVE already found for the host as remediated, so an unstable choice
...
func TestCandidateBetterThan(t *testing.T) {
	...
	// candidates happen to be compared in. This also covers the mirror of every
	// case above.
	all := []candidate{
		{pID: "1"},
