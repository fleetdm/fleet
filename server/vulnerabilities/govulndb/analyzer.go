...
// loadLatestArtifact reads the most recent mirrored artifact in vulnPath, or nil if the
// directory holds none. Artifact names sort by their date suffix.
func loadLatestArtifact(vulnPath string) (*Artifact, error) {
	entries, err := os.ReadDir(vulnPath)
	if err != nil {
		return nil, fmt.Errorf("reading %s: %w", vulnPath, err)
	}
	...
	for _, entry := range entries {
		...
		if isArtifactAsset(entry.Name()) && entry.Name() > latest {
			latest = entry.Name()
		}
	}
	if latest == "" {
	...
	return loadArtifact(filepath.Join(vulnPath, latest))
}
