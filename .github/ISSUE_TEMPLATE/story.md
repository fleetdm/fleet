...
- [ ] Exposed, public API endpoint and outbound requests changes: TODO <!-- Specify changes in the "Which API endpoints to expose to the public internet?" and "Outbound requests from Fleet" guides as a PR to reference docs release branch following the guidelines in the handbook here: https://fleetdm.com/handbook/product-design#drafting File a :help-customers request and assign the SVP of Customer Success. Up to Customer Success to device if any changes to cloud environments is needed.  Put "No changes" if there are no changes necessary. -->
