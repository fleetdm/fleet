...
The two features I want to focus on for this blog post are the live queries and the scheduled queries. Live queries are probably what you would assume they are. It is a feature where you can run a report from the Fleet application and get near-real-time results back from an endpoint (or many endpoints) very quickly. Scheduled queries run at a set increment of time, and those report results can be streamed from the application to cloud storage.

### Live report interface:
...
- osquery binary data and report results data go into two separate folders within the S3 bucket
...
The above report will result in giving us everything osquery can collect about Windows 10 applications. If you installed osquery on a Windows 10 device you can run the interactive mode binary and just test out the report locally. In Fleet for this scheduled report, we named it `windows-apps-ingest` which later downstream we can use as a primary key in Snowflake.

With Snowflake, I can simply run this report to grab the data I need. Remember, we are shipping the raw JSON results to a single schema/table in one column and then leveraging post-data processing features in Snowflake to get the data we need out of the raw JSON results. Here is a truncated sample of the data you will get from Fleet into Snowflake.
