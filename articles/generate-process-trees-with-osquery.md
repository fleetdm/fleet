...
Using advanced SQL syntax, it is possible to generate process trees in osquery similar to those generated by the `pstree` utility. With Fleet and osquery, the generated trees can be extended to include additional information that can aid analysis.

Below is the basic structure of the report:
...
## Wrapping Up
With this query as a building block, Fleet provides the capability to generate rich process trees. Consider using this with an osquery TLS server such as [Fleet](https://fleetdm.com/) to examine this information on multiple machines at once.
