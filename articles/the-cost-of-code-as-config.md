...
- **Providers.** Each platform needs at least one provider to source, version-pin, authenticate, and keep current. In the Jamf world, full coverage can mean running two side by side: the community Jamf Pro provider for classic configuration and Jamf's newer official platform provider for modern features, bridged by a data source that translates one object's ID into another's. The Workspace ONE provider, by contrast, is still labeled tech preview, with a deliberately narrow set of resources.  
