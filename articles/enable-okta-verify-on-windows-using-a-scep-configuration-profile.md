...
> **Note:** Use a static SCEP challenge, which is the best practice on Windows. Windows only accepts letters, numbers, spaces, and `' ( ) + , - . / : = ?` in a SCEP challenge. Okta's dynamic SCEP challenges sometimes include `_`, which Windows doesn't accept, so enrollment fails on some hosts. Fleet resends failed profiles, but that doesn't guarantee success. Automatically requesting a new challenge from Okta when this happens is tracked in [fleetdm/fleet#53882](https://github.com/fleetdm/fleet/issues/53882).
>
