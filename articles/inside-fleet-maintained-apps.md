...
Patch policies close the loop on hosts that fall behind. Add one from the app's details page under **Actions > Deploy**, enable **Patch**, and Fleet generates a policy that detects hosts running outdated versions. To enable the install automation, select **Patch when app is closed** or **Force patch**. Change this later at **Actions > Deploy** or **Policies > [policy] > Edit policy > Patch**. The patch policy's query updates itself to reference the latest version hourly or when your [GitOps](https://fleetdm.com/docs/configuration/yaml-files#patch-policy) specs are applied, so the policy never goes stale. The result is a chain with no manual links: the vendor releases, Fleet validates and publishes, your server syncs, and your hosts remediate.
