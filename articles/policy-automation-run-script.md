...
* Policy automation scripts are automatically attempted up to 3 total times. If the script exits with a non-zero exit code (i.e., the script execution fails), Fleet retries the script, up to a total of 3 attempts. If the script exits successfully (exit code 0), no retry occurs. If the host passes the policy, the retry count resets.
