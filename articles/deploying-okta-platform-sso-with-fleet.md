...
**Important:** You must also add an Organizational Unit (OU) field to the Subject with the value `$FLEET_VAR_SCEP_RENEWAL_ID`. This is required for Fleet to manage certificate renewals. In the raw XML, this appears as an additional array in the Subject:
...
Fleet replaces `$FLEET_VAR_NDES_SCEP_PROXY_URL`, `$FLEET_VAR_NDES_SCEP_CHALLENGE`, and `$FLEET_VAR_SCEP_RENEWAL_ID` with fresh values each time the profile is delivered to a host. Each host receives a unique, short-lived challenge rather than a shared static secret.
