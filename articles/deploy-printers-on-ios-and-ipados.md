...
You can add printers to iOS and iPadOS hosts with a configuration profile. iOS and iPadOS don't support installing a printer driver, so this only works for AirPrint-capable printers. The AirPrint payload populates a list of AirPrint printers for the Print dialog to show, it doesn't add support for printers that aren't AirPrint-capable.
...
Unlike the macOS Printing payload, you can deliver more than one AirPrint payload to the same host.

> **Note:** This is force-install only until [self-service configuration profiles](https://github.com/fleetdm/fleet/issues/46834), targeted for Fleet 4.93.0, ships.
...
1. On the host, open the **Print** dialog from any app and confirm the printer appears under **Printer**.
2. On the host's **Host details** page in Fleet, open the **OS settings** tab and confirm the profile shows as **Verified**.
...
- Apple's [AirPrint payload settings](https://support.apple.com/guide/deployment/airprint-payload-settings-dep3b4cf515/web).
