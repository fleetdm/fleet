...
When an organization maintains an approved software list, SQL-based querying makes it possible to compare what's actually present on devices against that list on a recurring basis, turning discovery from a one-time effort into a continuous process.
...
Combining device management with queryable device data makes shadow IT discovery practical across mixed operating system environments. Fleet uses osquery to provide SQL-based visibility into installed applications, many browser extensions, running processes, and network connections across macOS, Windows, and Linux from a single console. For SaaS discovery, combine device signals with identity provider and network logs.

Fleet also supports [GitOps workflows](https://fleetdm.com/fleet-gitops) for managing device configurations through version-controlled repositories, letting teams define policies alongside the rest of their infrastructure.[Request a demo](https://fleetdm.com/contact) to see how Fleet supports shadow IT discovery across your fleet.
