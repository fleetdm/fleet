...
Fleet stores as many results per report as you have hosts. This means a report that returns one row per host always covers your whole fleet, so you can use reports the way you'd use extension attributes in Jamf. As long as the report stays within this limit, Fleet updates it each time hosts send new data.
