...
- Replaces Zoom Fleet-maintained app with Zoom for IT, which does not open any windows during installation.
...
- Fixes a bug where the name of the setup experience script was not showing up in the activity for that script execution.
- Fixed bug in `fleetdm/fleetctl` docker image where the `build` directory does not exist when generating deb/rpm packages.
- Fixes a bug where the github cli software name was not matching against the cpe vulnerability name.
...
- Fixed an issue where the HTTP client used for MDM APNs push notifications did not support using a configured proxy.
